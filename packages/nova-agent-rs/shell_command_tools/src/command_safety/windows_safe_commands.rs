...
///
/// 解析由 tree-sitter 进程内完成，不再启动 PowerShell 子进程；
/// 未知或动态语法一律返回 None（fail-closed）。
...
mod tests {
    ...
    fn rejects_powershell_commands_with_side_effects() {
        ...
        // 括号包裹的命令表达式不在 tree-sitter 白名单 CST 形态内，fail-closed 拒绝
        //（对应 fixture 中的 parenthesized_command/parenthesized_pipeline 用例）。
        assert!(!is_safe_command_windows(&vec_str(&[
            "powershell.exe",
            "-Command",
    ...
    #[test]
    fn accepts_pipeline_chains_independent_of_powershell_variant() {
        // tree-sitter 进程内解析不再按 powershell.exe / pwsh.exe 变体区分语法面：
        // `&&`/`||` 链式形态统一按 pipeline_chain 降维，逐条命令过只读白名单。
        //（subprocess AST 时代 powershell.exe 5.1 不认识 `&&`，整条拒绝；该行为已移除。）
        let chain = "pwd && ls";
        assert!(
            ...
            "`{chain}` lowers to two read-only commands via tree-sitter"
        );
