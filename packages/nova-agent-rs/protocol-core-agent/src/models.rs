...
use nova_executor_utils_image::PromptImageMode;
use nova_executor_utils_image::data_url_from_bytes;
use nova_executor_utils_image::load_for_prompt_bytes;
...
- ["a", "b", "c"]"#
    ...
    fn format_allow_prefixes_limits_output() {
        // 直接构造前缀列表（execpolicy 已挪出 executor 仓库——存档于
        // packages/nova-agent-rs/）：200 条长前缀触发字节数截断逻辑
        let prefixes = (0..200)
