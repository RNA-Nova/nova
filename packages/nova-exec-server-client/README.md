# nova-exec-server-client

`nova-exec-server-client` 是 `nova-exec-server` 的 Python SDK——**executor 栈的客户端运行时**（对位 codex `codex-exec-server` crate 的客户端半边）。
...
- **不做部署**：不负责上传、安装、启动 nova-exec-server 到服务器
...
```sh
pip install nova-exec-server-client
```
...
```python
# 本地：等价于 spawn ["nova-exec-server", "--listen", "stdio"]
async with ExecutorClient.from_stdio() as client:
    ...

# SSH 远程：spawn ["ssh", "user@host", "nova-exec-server", "--listen", "stdio"]
...
from nova_exec_server_client import StdioTransport
client = ExecutorClient(transport=StdioTransport(program="/path/to/nova-exec-server"))
```
...
```python
from nova_exec_server_client import load_executor_config, resolve_execution_policy
...
```python
from nova_exec_server_client import ExecutorClient, NetworkPolicyGate, AskOutcome
...
```python
from nova_exec_server_client import load_executor_config, resolve_environment, ExecutorClient
...
```python
from nova_exec_server_client import EnvironmentManager
...
```python
from nova_exec_server_client import ExecutorClient, ReconnectStrategy
...
本 SDK **不负责部署** nova-exec-server。你需要：

1. 在服务器上编译或下载 `nova-exec-server` 二进制
2. 启动服务：`nova-exec-server --listen ws://0.0.0.0:8080 --auth bearer --auth-token your-token`（stdio 模式为 `nova-exec-server --listen stdio`，单连接，由 SDK 直接 spawn 时无需手动启动）
3. 使用 SDK 连接
...
# 仓库 target/debug/nova-exec-server → PATH；找不到自动 skip）
