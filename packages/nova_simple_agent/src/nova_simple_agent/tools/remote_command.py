...
from pi_agent import AgentToolResult  # 根据你的框架调整导入
...
class RemoteCommandTool(RemoteTool):
    ...
    def __init__(self):
        ...
        self.label = "远程命令执行"
        self._initialized_hosts: Set[str] = set()
    
    def _get_host_key(self, host: str, port: int) -> str:
        """获取主机唯一标识"""
        return f"{host}:{port}"
    
    async def _init_host_environment(self, host: str, port: int) -> None:
        """
        初始化远程主机的运行环境
        加载bash配置，只在第一次连接时执行
        """
        ...
        # 如果已经初始化过，跳过
        if host_key in self._initialized_hosts:
            return
        
        try:
            proxy = self._get_proxy(host, port)
            proxy.run("bash", "source ~/.bashrc")
            self._initialized_hosts.add(host_key)
        except Exception as e:
            # 初始化失败时，记录错误但继续，因为后续命令可能会失败
            # 可以选择重试或直接失败，这里选择继续但记录警告
            print(f"警告：初始化远程主机 {host}:{port} 失败: {e}")
            # 如果初始化失败，不加到已初始化集合，下次可能重试
    ...
    async def execute(self, tool_call_id: str, params: Dict[str, Any], 
        ...
        try:
            # 初始化远程主机环境
            await self._init_host_environment(host, port)
            
            # 使用基类提供的代理方法
            proxy = self._get_proxy(host, port)
            
            # 执行run方法，参数顺序是 (language, command)
            results = proxy.run(language, command)
