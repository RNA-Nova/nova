...
const READY_MARKER: &str = "__NOVA_EXEC_SERVER_CHILD_READY__";
const VALUE_MARKER: &str = "__NOVA_EXEC_SERVER_CHILD_VALUE__";
...
async fn conpty_delivers_input_to_foreground_children() -> anyhow::Result<()> {
    ...
    let code = format!(
        "print('__NOVA_EXEC_SERVER_CHILD_'+'READY__', flush=True); value=input(); print('{VALUE_MARKER}'+value.encode('utf-8').hex(), flush=True)"
    );
