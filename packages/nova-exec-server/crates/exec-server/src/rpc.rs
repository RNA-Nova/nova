...
use nova_exec_server_protocol::JSONRPCError;
...
/// outbound channel 为控制面消息（响应等）保留的容量格数：
/// best-effort 通知（`try_notify`）在剩余容量不足时直接丢弃，不得饿死控制面响应。
...
impl RpcNotificationSender {
    ...
    /// best-effort 通知：非阻塞 `try_reserve` 预留一格容量后发送。
    ///
    /// 预留后若 channel 剩余容量不足 `RESERVED_OUTBOUND_CONTROL_MESSAGES`
    /// （须给响应等控制面消息保留的格数）则放弃发送并返回 `false`，
    /// 保证通知再密集也饿不死控制面响应。
    ///
    /// 仅用于真正可丢的通知；fs/readStream 的 chunk/done 推送刻意继续使用
    /// 阻塞 `notify`（数据完整性优先，丢不得）。当前消费者：托管网络代理的
    /// `network/policyDecision` 审计通知（丢失只影响审计完整性，不影响裁决本身）。
...
mod tests {
    ...
    // best-effort 通知不得吃掉为控制面保留的容量：容量不足时丢弃通知，响应仍可发出。
    #[tokio::test]
    async fn best_effort_notifications_preserve_outbound_control_capacity() {
        let (outgoing_tx, _outgoing_rx) =
            tokio::sync::mpsc::channel(RESERVED_OUTBOUND_CONTROL_MESSAGES + 2);
        let notifications = RpcNotificationSender::new(outgoing_tx);

        assert!(notifications.try_notify("test/bestEffort", &serde_json::json!({"n": 1})));
        assert!(notifications.try_notify("test/bestEffort", &serde_json::json!({"n": 2})));
        assert!(!notifications.try_notify("test/bestEffort", &serde_json::json!({"n": 3})));
        notifications
    ...
    async fn inbound_request_span_stays_open_until_event_consumption() {
        ...
        let tracer_provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer_provider_tracer = tracer_provider.tracer("exec-server-test");
        let subscriber = tracing_subscriber::registry().with(
            tracing_opentelemetry::layer()
                .with_tracer(tracer_provider_tracer)
                .with_filter(filter_fn(
                    nova_exec_server_otel::OtelProvider::trace_export_filter,
                )),
        );
