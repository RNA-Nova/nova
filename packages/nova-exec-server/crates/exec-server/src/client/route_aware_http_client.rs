...
use http::Method;
use nova_exec_server_http_client::ClientRouteClass;
use nova_exec_server_http_client::HttpClientFactory;
use nova_exec_server_http_client::RouteAwareClientPool;
use nova_exec_server_http_client::RouteAwareRequestError;
...
pub(crate) struct PendingRouteAwareHttpBodyStream {
    ...
    pub(crate) response: nova_exec_server_http_client::HttpResponse,
}
...
impl RouteAwareHttpRequestRunner {
    ...
    ) -> Result<(HttpRequestResponse, Option<PendingRouteAwareHttpBodyStream>), JSONRPCErrorError>
    {
        ...
        nova_exec_server_otel::inject_span_w3c_trace_headers(&request_span, &mut headers);
