...
use crate::environment::NOVA_EXEC_SERVER_URL_ENV_VAR;
use crate::environment::REMOTE_ENVIRONMENT_ID;

/// Lists the remote environment transports available to the Nova runtime.
///
...
/// Default provider backed by `NOVA_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `NOVA_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `NOVA_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(NOVA_EXEC_SERVER_URL_ENV_VAR).ok())
    }
