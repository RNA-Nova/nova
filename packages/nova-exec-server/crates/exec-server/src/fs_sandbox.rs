...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let command = SandboxCommand {
            ...
            args: vec![NOVA_EXEC_SERVER_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                windows_sandbox_proxy_settings_mode:
                    nova_exec_server_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| {
        !nova_exec_server_protocol_core::shell_environment::is_non_inheritable_env_var(name)
    });
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            nova_exec_server_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context =
            nova_exec_server_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let executor_self_exe = root.path().join("bin").join("nova-exec-server");
    ...
    fn sandbox_context_with_cwd(
        ...
        nova_exec_server_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
