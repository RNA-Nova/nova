...
use nova_exec_server::ExecBackend;
#[cfg(unix)]
use nova_exec_server::ExecEnvPolicy;
use nova_exec_server::ExecOutputStream;
use nova_exec_server::ExecParams;
use nova_exec_server::ExecProcess;
use nova_exec_server::ExecProcessEvent;
#[cfg(any(unix, windows))]
use nova_exec_server::FileSystemSandboxContext;
use nova_exec_server::LocalProcess;
use nova_exec_server::ProcessId;
use nova_exec_server::ProcessSignal;
use nova_exec_server::ReadResponse;
use nova_exec_server::RemoteProcess;
#[cfg(unix)]
use nova_exec_server::ShellInfo;
#[cfg(unix)]
use nova_exec_server::ShellSnapshotRequest;
use nova_exec_server::StartedExecProcess;
use nova_exec_server::WriteStatus;
...
async fn shell_snapshot_v2_filters_profile_exports_and_stays_in_memory(
    ...
    let command = format!(
        "export PATH='{}':\"$PATH\"; {command_prefix}printf '|%s|%s|%s|%s|%s|%s' \"$PROFILE_ALLOWED\" \"${{PROFILE_SECRET-missing}}\" \"${{PROFILE_DENIED-missing}}\" \"$PATH\" \"${{__NOVA_EXEC_SERVER_SHELL_SNAPSHOT_STATE_0-missing}}\" \"${{__NOVA_EXEC_SERVER_SHELL_SNAPSHOT_STATE_1-missing}}\"",
    ...
    if let Some(server) = context._server {
        assert!(!server.nova_exec_server_home().join("shell_snapshots").exists());
    }
    Ok(())
}
...
async fn shell_snapshot_v2_remote_managed_proxy_uses_prepared_execution_context() -> Result<()> {
    ...
    std::fs::write(
        ...
        "printf '%s\\n' \"$HTTP_PROXY\" >> \"$HOME/captures\"\ntest \"$NOVA_EXEC_SERVER_NETWORK_PROXY_ACTIVE\" = 1 || exit 41\nexport PROFILE_ALLOWED=profile\nprofile_helper() { printf helper; }\n",
    )?;
    let policy = ExecEnvPolicy {
        inherit: ShellEnvironmentPolicyInherit::All,
        ignore_default_excludes: false,
    ...
    for attempt in 0..2 {
        let started = context
            ...
            .start(ExecParams {
                ...
                argv: vec![
                    "/bin/bash".to_string(),
                    "-lc".to_string(),
                    "profile_helper; printf '|%s|%s|%s' \"$PROFILE_ALLOWED\" \"$NOVA_EXEC_SERVER_NETWORK_PROXY_ACTIVE\" \"$HTTP_PROXY\"".to_string(),
                ],
                cwd: cwd.clone(),
                env_policy: Some(policy.clone()),
                shell_snapshot: Some(ShellSnapshotRequest {
        ...
        let (stdout, stderr, status, closed) =
            collect_process_output_from_events(started.process).await?;
        let Some(proxy_address) = stdout.strip_prefix("helper|profile|1|") else {
            panic!(
                "snapshot should restore profile functions and live proxy state: stdout={stdout:?} stderr={stderr:?} status={status:?} closed={closed:?}"
            );
        };
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            argv: vec![
                "/bin/sh".to_string(),
                "-c".to_string(),
                "printf '%s' \"$0\"; if /bin/cat \"$NOVA_EXEC_SERVER_TEST_DENIED_FILE\" >/dev/null 2>&1; then exit 42; fi"
                    .to_string(),
            ],
            cwd,
            shell_snapshot: None,
            ...
            env: HashMap::from([
                ...
                (
                    "NOVA_EXEC_SERVER_TEST_DENIED_FILE".to_string(),
