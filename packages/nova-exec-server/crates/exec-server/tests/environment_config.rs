...
async fn exec_server_reads_environment_config_layers() -> anyhow::Result<()> {
    ...
    // user 层：<executor home>/config.toml（harness 已把 NOVA_EXEC_SERVER_HOME
    // 指向临时目录）
    std::fs::write(
        server.nova_exec_server_home().join("config.toml"),
        r#"
[sandbox]
...
"#,
    ...
    let cwd = nova_exec_server_utils_path_uri::PathUri::from_host_native_path(project_root.path())?;
    let read_id = server
        .send_request(
    ...
    // find_nova_exec_server_home 对 NOVA_EXEC_SERVER_HOME 做 canonicalize（macOS 上
    // /var → /private/var；windows 上会带 `\\?\` verbatim 前缀而服务端输出
    // 不带），断言期望值同样规整后再比对。
    #[allow(unused_mut)]
    let mut expected_home = server.nova_exec_server_home().canonicalize()?;
    #[cfg(windows)]
    {
    ...
    assert_eq!(
        ...
        serde_json::json!(
            nova_exec_server_utils_path_uri::PathUri::from_host_native_path(&expected_home)?
                .to_string()
        )
    );
...
async fn exec_server_environment_config_read_rejects_empty_selectors() -> anyhow::Result<()> {
    ...
    let cwd = nova_exec_server_utils_path_uri::PathUri::from_host_native_path(
        &std::env::temp_dir(),
    )?;
    for config_paths in [
        serde_json::json!([]),
        ...
        let request_id = server
            ...
            .await?;
        let JSONRPCMessage::Error(nova_exec_server_protocol::JSONRPCError {
            id, error, ..
