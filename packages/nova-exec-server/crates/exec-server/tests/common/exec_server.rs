    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let nova_exec_server_home = TempDir::new()?;
    ...
    child.env("NOVA_EXEC_SERVER_HOME", nova_exec_server_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn nova_exec_server_home(&self) -> &std::path::Path {
        self.nova_exec_server_home.path()
    }
