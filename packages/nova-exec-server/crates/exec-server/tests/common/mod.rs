//! 集成测试共享夹具：测试二进制经 ctor 兼任 nova-exec-server 服务器与隐藏 helper。
//!
...
//!   `NOVA_EXEC_SERVER_HOME`（见 `exec_server.rs`）。
...
use nova_exec_server::ExecServerRuntimePaths;
use nova_exec_server::ExecServerTelemetry;
use nova_exec_server::NOVA_EXEC_SERVER_ARG0_EXEC_HELPER_ARG1;
use nova_exec_server::NOVA_EXEC_SERVER_FS_HELPER_ARG1;
use nova_exec_server::RequestDispatchMode;
...
pub(crate) const SYSTEM_PROXY_REQUEST_URL_ENV: &str = "NOVA_EXEC_SERVER_TEST_SYSTEM_PROXY_REQUEST_URL";
pub(crate) const SYSTEM_PROXY_URL_ENV: &str = "NOVA_EXEC_SERVER_TEST_SYSTEM_PROXY_URL";
...
static TEST_BINARY_DISPATCH: () = {
    ...
    if let Some(program) = program.as_deref() {
        let is_linux_sandbox = Path::new(program).file_name().is_some_and(|name| {
            name == nova_exec_server_sandboxing::landlock::NOVA_EXEC_SERVER_LINUX_SANDBOX_ARG0
        });
        if is_linux_sandbox {
            // run_main 的 panic（如 bwrap 缺失）若穿出 ctor 的 extern "C" 边界
            // 会变成无诊断信息的 SIGABRT；拦下换成可读错误再退出
            if std::panic::catch_unwind(nova_exec_server_linux_sandbox::run_main).is_err() {
    ...
    if argv1 == NOVA_EXEC_SERVER_ARG0_EXEC_HELPER_ARG1 {
        nova_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == NOVA_EXEC_SERVER_FS_HELPER_ARG1 {
        nova_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == NOVA_WINDOWS_SANDBOX_ARG1 {
        nova_exec_server_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
...
pub(crate) async fn connect_remote_exec_client(
    websocket_url: &str,
) -> anyhow::Result<nova_exec_server::ExecServerClient> {
    use std::time::Duration;

    Ok(nova_exec_server::ExecServerClient::connect_websocket(
        nova_exec_server::RemoteExecServerConnectArgs {
...
/// 与 `nova-exec-server` CLI 的本地模式等价，但 runtime paths 指向测试二进制，
...
fn run_exec_server_from_test_binary(mut args: impl Iterator<Item = std::ffi::OsString>) -> ! {
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            nova_exec_server_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(nova_exec_server::run_main_with_telemetry(
        &listen_url,
        runtime_paths,
