...
use nova_exec_server::HttpHeader;
use nova_exec_server::HttpRedirectPolicy;
use nova_exec_server::HttpRequestBodyDeltaNotification;
use nova_exec_server::HttpRequestParams;
use nova_exec_server::HttpRequestResponse;
use nova_exec_server::InitializeParams;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
    ...
    // valueEnvVar 头由执行端环境变量填值（凭据不跨线委派）——
    // 非保护名单内的变量名才允许（NODE_REPL_AUTH_TOKEN/LINEAR_API_KEY 为
    // 任意非敏感名）。
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<nova_exec_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
...
/// 保护名单：执行端敏感环境变量不许经 valueEnvVar 代发（对位 codex
/// exec_server_http_request_rejects_protected_environment_headers）——
/// 名单内一律 -32602 拒绝且本地监听零连接（请求没有真的发出去）。
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_server_http_request_rejects_protected_environment_headers() -> anyhow::Result<()> {
    let mut server = exec_server_with_env(
        [(
            "NOVA_EXEC_SERVER_PROXY_ATTRIBUTION_TOKEN",
