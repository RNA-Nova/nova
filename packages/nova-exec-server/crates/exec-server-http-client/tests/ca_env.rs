...
//! detailed explanation of what "hermetic" means here lives in `nova_exec_server_http_client::custom_ca`; these
...
use nova_exec_server_utils_cargo_bin::cargo_bin;
...
const NOVA_EXEC_SERVER_CA_CERT_ENV: &str = "NOVA_EXEC_SERVER_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "NOVA_EXEC_SERVER_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "NOVA_EXEC_SERVER_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "NOVA_EXEC_SERVER_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(NOVA_EXEC_SERVER_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    nova_exec_server_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn uses_nova_ca_cert_env() {
    ...
    let output = run_probe(&[(NOVA_EXEC_SERVER_CA_CERT_ENV, cert_path.as_path())]);
...
fn prefers_nova_ca_cert_over_ssl_cert_file() {
    ...
    let output = run_probe(&[
        (NOVA_EXEC_SERVER_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(NOVA_EXEC_SERVER_CA_CERT_ENV, cert_path.as_path())], &server.url);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("NOVA_EXEC_SERVER_CA_CERTIFICATE"));
