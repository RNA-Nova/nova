...
use nova_exec_server_utils_pty::SpawnedProcess;
use nova_exec_server_utils_pty::TerminalSize;
...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            return nova_exec_server_windows_sandbox::spawn_windows_sandbox_session_for_level(
                nova_exec_server_windows_sandbox::WindowsSandboxSessionRequest {
    ...
    if request.tty {
        ...
        nova_exec_server_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        nova_exec_server_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,
