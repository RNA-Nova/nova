...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        nova_exec_server_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(NOVA_EXEC_SERVER_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        NOVA_EXEC_SERVER_LINUX_SANDBOX_ARG0.to_string()
    }
}
