...
use nova_exec_server_protocol_core::models::NetworkPermissions;
...
fn transform_additional_permissions_preserves_denied_entries() {
    ...
    assert_eq!(
        ...
        FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: denied_path.into() },
                access: FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            },
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: allowed_path.into() },
...
fn managed_mitm_ca_bundle_becomes_readable_for_restricted_sandbox() {
    ...
    assert_eq!(
        ...
        FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: cwd.into() },
                access: FileSystemAccessMode::Read,
                missing_path_behavior: None,
            },
            FileSystemSandboxEntry {
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let nova_linux_sandbox_exe = std::path::PathBuf::from("/tmp/nova-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&nova_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(nova_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_for_direct_spawn_windows_materializes_inner_helper() {
    ...
    let manager = SandboxManager::new();
    let exec_request = manager
        .transform_for_direct_spawn_with_executor_home(
