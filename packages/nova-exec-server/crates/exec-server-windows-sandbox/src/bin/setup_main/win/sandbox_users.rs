...
use nova_exec_server_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "NovaSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Nova sandbox internal group (managed)";
...
pub(super) fn prepare_setup_marker(sandbox_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(sandbox_home).join("setup_marker.json");
