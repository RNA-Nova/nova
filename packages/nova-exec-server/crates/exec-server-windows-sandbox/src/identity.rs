...
pub fn sandbox_setup_is_complete(sandbox_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(sandbox_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
...
pub fn require_logon_sandbox_creds(
    ...
    // NOTE: Do not add NOVA_EXEC_SERVER_HOME/.sandbox to `needed_write`; it must remain non-writable by the
