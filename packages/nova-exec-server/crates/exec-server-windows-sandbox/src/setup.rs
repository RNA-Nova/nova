...
const SETUP_EXE_FILENAME: &str = "nova-windows-sandbox-setup.exe";
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "NOVA_EXEC_SERVER_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "NOVA_EXEC_SERVER_WINDOWS_SANDBOX_PROXY_PORTS";
...
pub fn run_elevated_provisioning_setup(
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning NOVA_EXEC_SERVER_HOME must be an absolute local disk path: {}",
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sandbox_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to NOVA_EXEC_SERVER_HOME or anything under NOVA_EXEC_SERVER_HOME/.sandbox,
    // NOVA_EXEC_SERVER_HOME/.sandbox-bin, or NOVA_EXEC_SERVER_HOME/.sandbox-secrets. These locations contain sandbox
...
mod tests {
    ...
    use nova_exec_server_protocol_core::permissions::project_roots_glob_pattern;
    use nova_exec_server_utils_absolute_path::AbsolutePathBuf;
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "NOVA_EXEC_SERVER_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
