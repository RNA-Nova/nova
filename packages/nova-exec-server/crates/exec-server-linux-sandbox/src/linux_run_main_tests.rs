...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/nova-arg0-session/nova-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/nova-arg0-session/nova-linux-sandbox"] })
    );
}
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        nova_exec_server_protocol_core::permissions::FileSystemSandboxEntry {
            path: nova_exec_server_protocol_core::permissions::FileSystemPath::Special {
                value: nova_exec_server_protocol_core::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: nova_exec_server_protocol_core::permissions::FileSystemAccessMode::Write,
            missing_path_behavior: None,
        },
        nova_exec_server_protocol_core::permissions::FileSystemSandboxEntry {
            path: docs.into(),
            access: nova_exec_server_protocol_core::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
        },
    ]);
...
fn root_write_read_only_carveout_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        nova_exec_server_protocol_core::permissions::FileSystemSandboxEntry {
            path: nova_exec_server_protocol_core::permissions::FileSystemPath::Special {
                value: nova_exec_server_protocol_core::permissions::FileSystemSpecialPath::Root,
            },
            access: nova_exec_server_protocol_core::permissions::FileSystemAccessMode::Write,
            missing_path_behavior: None,
        },
