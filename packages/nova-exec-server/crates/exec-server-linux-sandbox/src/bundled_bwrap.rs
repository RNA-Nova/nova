...
fn legacy_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("nova-resources").join("bwrap"));
...
fn expected_sha256() -> Option<[u8; 32]> {
    ...
    *EXPECTED.get_or_init(|| {
        let raw_digest = option_env!("NOVA_EXEC_SERVER_BWRAP_SHA256")?;
        let digest = parse_sha256_hex(raw_digest)
            .unwrap_or_else(|err| panic!("invalid NOVA_EXEC_SERVER_BWRAP_SHA256 value: {err}"));
...
mod tests {
    ...
    fn finds_legacy_standalone_bundled_bwrap_next_to_exe_resources() {
        ...
        let exe = temp_dir.path().join("nova-exec-server");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let exe = target_dir.join("nova-exec-server").join("nova-exec-server");
