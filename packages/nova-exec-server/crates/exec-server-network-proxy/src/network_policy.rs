//! 网络策略评估与审计事件（移植自 codex network-proxy `network_policy.rs`）。
//!
//! 策略类型本体（`NetworkProtocol` / `NetworkPolicyRequest` / `NetworkDecision` /
//! `NetworkPolicyDecider` 等）定义在 crate 根 lib.rs（线上类型的家），本模块承载行为。
//!
//! 适配点：
//! - `NetworkProxyAuditMetadata` 线上形状只有 `conversation_id` / `turn_id` 两个字段
//!   （codex 端为九个）；审计事件相应只发 `conversation.id` / `turn.id`。
//! - `NetworkPolicyRequest` 无 codex 端 `disconnect` 字段（`request_disconnect.rs`
//!   第一期未移植）。
//! - 遥测 target / 事件名沿用 codex 前缀（与 executor-otel 的 `codex.*` 指标名先例一致：
//!   遥测契约不随 crate 改名）。
...
const AUDIT_TARGET: &str = "nova_exec_server_otel.network_proxy";
...
/// 观察最终的网络策略决策，不拖延也不改变执行。
///
/// 实现必须立即返回，并把通知投递视为尽力而为。
...
/// 判定一次网络请求是否放行。
///
/// 如果提供了 `command` 或 `exec_policy_hint`，调用方可以把 exec-policy 批准映射为
/// 网络访问（例如对匹配已批准前缀如 `curl *` 的命令放行全部请求）。
///
/// 注：`NetworkPolicyDecider` trait 与 `NetworkDecision` 的构造器在 crate 根定义。
...
pub(crate) mod test_support {
        ...
        Fut: Future<Output = T>,
    {
        ...
        // 让 tracing 离开单 subscriber 快速路径：否则无 subscriber 的并发测试
        // 可能把该 callsite 缓存为 disabled。
...
mod tests {
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        let decider: Arc<dyn NetworkPolicyDecider> = Arc::new({
            ...
            move |_req| {
                ...
                // 默认策略全拒；decider 仅在 not_allowed 时被咨询，可覆盖该决策。
        ...
        assert!(event.target.starts_with("nova_exec_server_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        // 适配：nova 线上 metadata 只有 conversation_id / turn_id 两个字段。
        ...
        assert_eq!(event.field("turn.id"), Some("turn-1"));
    }
