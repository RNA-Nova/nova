...
#[command(name = "nova-exec-server-network-proxy", about = "Nova network sandbox proxy")]
pub struct Args {}
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "NOVA_EXEC_SERVER_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "NOVA_EXEC_SERVER_NETWORK_ALLOW_LOCAL_BINDING";
...
///
/// 相对 stub 快照补齐了 `NOVA_EXEC_SERVER_PROXY_ATTRIBUTION_TOKEN`、`ELECTRON_GET_USE_PROXY`、
...
#[cfg(target_os = "macos")]
pub const NOVA_PROXY_GIT_SSH_COMMAND_MARKER: &str = "NOVA_EXEC_SERVER_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const NOVA_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "NOVA_EXEC_SERVER_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"NOVA_EXEC_SERVER_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
        #[cfg(not(target_os = "macos"))]
