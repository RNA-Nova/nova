...
use nova_exec_server::run_main_with_telemetry;

/// nova-exec-server - 远程执行服务
#[derive(Debug, Parser)]
#[command(name = "nova-exec-server")]
...
struct Cli {
    ...
    #[arg(long, env = "NOVA_EXEC_SERVER_EXEC_SERVER_EXIT_ON_STDIN_CLOSE")]
...
fn assemble_otel_provider() -> Option<nova_exec_server_otel::OtelProvider> {
    let home = match nova_exec_server_utils_home_dir::find_nova_exec_server_home() {
        Ok(home) => home,
        Err(error) => {
    ...
    let settings = match nova_exec_server_otel::load_otel_settings(home.as_path()) {
    ...
    match nova_exec_server_otel::OtelProvider::try_new(&settings) {
...
fn main() -> Result<()> {
    ...
    match args.next().as_deref() {
        ...
        Some(arg)
            if arg == std::ffi::OsStr::new(nova_exec_server::NOVA_EXEC_SERVER_ARG0_EXEC_HELPER_ARG1) =>
        {
...
async fn run_server() -> Result<()> {
    ...
    let request_dispatch_mode = if cli.concurrent_requests <= 1 {
        ...
        RequestDispatchMode::Concurrent {
            max_concurrent_requests: nova_exec_server::ConcurrentRequestLimit::new(
    ...
    run_main_with_telemetry(
        &cli.listen,
        runtime_paths,
        nova_exec_server::ExecServerTelemetry::default(),
