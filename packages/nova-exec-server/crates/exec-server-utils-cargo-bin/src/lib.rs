...
/// site.)
///
/// This is expected to be used exclusively in test code because nova-exec-server is a
...
pub fn repo_root() -> io::Result<PathBuf> {
    let marker = if runfiles_available() {
        ...
        let marker_path = option_env!("NOVA_EXEC_SERVER_REPO_ROOT_MARKER")
            .map(PathBuf::from)
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    "NOVA_EXEC_SERVER_REPO_ROOT_MARKER was not set at compile time",
                )
            })?;
