...
export interface RendererInput {
  ...
  /**
   * 参数已完整、执行未开始（streaming 态内的时点标记）——
   * edit 类渲染器的执行前只读预览（diff）在这个时点产出。
   */
   ...
   */
  preview?: unknown;
  partial?: RendererResultPart;
  result?: RendererResultPart;
...
/** 读取 details（result 优先，partial 兜底）。 */
export function detailsOf(input: RendererInput): Record<string, unknown> {
  return {
    ...asRecord(input.partial?.details),
    ...asRecord(input.result?.details),
  };
}
