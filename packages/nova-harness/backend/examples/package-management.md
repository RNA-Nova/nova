...
> 本文档面向 Nova 开发者与高级用户，说明 `nova_harness.package` 的设计路线、安装语义、目录布局与典型用法。
...
- `tools/` 下每个含 `executor.py` 的目录（`Tool` 类，元数据为类属性）。
- `user_tools/` 下每个含 `executor.py` 的目录（`UserTool` 类）。
...
TypeScript 版 `coding-agent` 的包管理是单文件 `package-manager.ts`，功能集中；Python 版 `package/` 按职责拆分为：
