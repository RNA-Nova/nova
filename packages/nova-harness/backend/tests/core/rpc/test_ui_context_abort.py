...
async def test_timeout_ms_overrides_default_timeout(ui_env):
    ...
    assert resp.cancelled is True
    assert elapsed < 1.0  # 50ms 级超时，而非 300s 默认


@pytest.mark.asyncio
async def test_signal_abort_sends_ui_cancel_and_resolves(ui_env):
    """signal abort → 向前端发 ui/cancel 撤销帧 + 请求按 cancelled 解决。"""
    registry, ui = ui_env
    _, client = _make_conn(registry, {"select"})
    controller = AbortController()

    task = asyncio.create_task(ui.request("select", {"title": "t"}, controller.signal))
