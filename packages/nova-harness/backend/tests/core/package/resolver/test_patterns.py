...
def test_normalize_path_for_compare_uses_base_dir(tmp_path) -> None:
    ...
    from nova_harness.core.package.resolve.discovery import _normalize_path_for_compare
...
def test_matches_exact_pattern_does_not_match_basename(tmp_path) -> None:
    """+/- 强制模式只比精确路径（绝对或相对 base_dir），不按 basename 命中（对齐 TS）。"""
    base = str(tmp_path)
    skill_dir = tmp_path / "skills" / "foo"
    skill_dir.mkdir(parents=True)

    assert matches_exact_pattern(str(skill_dir), "skills/foo", base) is True
    assert matches_exact_pattern(str(skill_dir), str(skill_dir), base) is True
    # basename 不再命中
    assert matches_exact_pattern(str(skill_dir), "foo", base) is False
    # SKILL.md 文件仍可按父目录精确指定
    skill_file = skill_dir / "SKILL.md"
    skill_file.write_text("x", encoding="utf-8")
    assert matches_exact_pattern(str(skill_file), "skills/foo", base) is True
    assert matches_exact_pattern(str(skill_file), "foo", base) is False
