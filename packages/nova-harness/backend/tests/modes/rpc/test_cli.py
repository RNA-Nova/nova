...
async def test_async_main_runs_server():
    """_async_main 应创建服务器、接入 stdio 连接、注册信号处理器并运行。"""
    mock_server = MagicMock()
    mock_server.add_connection = AsyncMock()
    ...
    assert result == 0
    mock_server.add_connection.assert_awaited_once()
    mock_server.run.assert_awaited_once()
    # SIGINT/SIGTERM（+ POSIX 的 SIGHUP）
    assert mock_loop.add_signal_handler.call_count == _EXPECTED_SIGNAL_COUNT
    assert mock_loop.remove_signal_handler.call_count == _EXPECTED_SIGNAL_COUNT
...
async def test_async_main_calls_shutdown_on_signal():
    ...
    for sig, (cb, args) in handlers.items():
        cb(*args)
    assert mock_server.shutdown.call_count == _EXPECTED_SIGNAL_COUNT
