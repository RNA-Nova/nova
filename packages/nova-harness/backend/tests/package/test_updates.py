...
def test_check_for_available_updates_detects_git_update(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nova_harness.package.install.updates._git_local_head", fake_local_head
    )
    monkeypatch.setattr(
        "nova_harness.package.install.updates._git_remote_head", fake_remote_head
    )
...
def test_check_for_available_updates_no_update(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nova_harness.package.install.updates._git_local_head", lambda d: head
    )
    monkeypatch.setattr(
        "nova_harness.package.install.updates._git_remote_head",
        lambda d, ref: head,
    )
...
def test_check_for_available_updates_swallows_check_errors(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nova_harness.package.install.updates._git_local_head",
