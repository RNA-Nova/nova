...
def test_update_all_plain(mock_pm_class, capsys):
    ...
    mock_pm.update_all = AsyncMock(return_value=[_metadata("x", "2.0")])
    mock_pm_class.return_value = mock_pm

    result = main(["update"])
    assert result == 0
    mock_pm.update_all.assert_awaited_once_with()
    mock_pm.update.assert_not_awaited()
    assert "Updated 'x'" in capsys.readouterr().out
