...
class AgentSession:
    ...
    def __init__(self, config: AgentSessionConfig) -> None:
        ...
        # 当前 run 身份（作用域仲裁的归属键）：agent_start 时由
        # EventController 生成、agent_end 时清空；挂起 UI 请求按
        # run:<id>/session:<id> 记账（ScopedUIContext 织入）
    ...
    def _current_scope(self) -> str:
        """当前作用域归属（仲裁记账键）：run 进行中归 ``run:<id>``，
        否则归本会话 ``session:<id>``——run 外的会话级交互（trust 询问、
        /tools 面板）不被 run 终结误杀。"""
        if self._current_run_id:
            return f"run:{self._current_run_id}"
    ...
    # -------------------------------------------------------------------------
    # item 直通（包级变体发射面——不透明摆渡）
    # -------------------------------------------------------------------------

    def emit_item_started(self, item: Any) -> None:
        """包级 item 发射：早建卡片（线上经 server 归约器转为 item_started）。

        包级变体（如 bash 执行卡片）在执行起点发射早建，执行中经
        ``emit_item_delta`` 增量，完成时**照常走 record 消息路径**——
        server 归约器在 message_start 处用消息的 ``to_item()`` 定稿
    ...
    def emit_item_delta(self, item_id: str, delta: Dict[str, Any]) -> None:
        """item 瞬态增量（不落盘；合并规则：字符串追加、其余替换）。"""
        self._emit(ItemEmissionEvent(phase="delta", item_id=item_id, delta=delta))
