...
class SlashInputHandler:
    ...
    async def execute_command(self, text: str) -> bool:
        ...
        被命令过滤（agent.yaml ``commands`` 允许集 / settings
        ``disabled_commands`` 排除集）拦截的命令：消费输入但不执行——
        经 UI 通知告知（不静默吞、不透传给 LLM）。
        """
        ...
        if not self._session._is_command_allowed(command.name):
            ui_context = getattr(runner, "ui_context", None)
            if ui_context is not None:
                # 经反向原语通知（headless 下 NoOp 自动静默）
                ui_context.notify(
                    "notify",
                    {
                        "message": f"命令 /{command.name} 已被当前 agent 配置或用户设置禁用",
                        "type": "warning",
                    },
                )
            return True
