...
框架不内置任何用户工具，其消息类定义在各自的包里。会话 JSONL 反序列化时
按 ``role`` 查本注册表复原消息类型：
...
注册发生在用户工具加载时（``resources/loaders/user_tools.py``），早于
``SessionManager`` 读取 JSONL——资源加载在 services 创建阶段完成，
JSONL 解析在 AgentSession 构造阶段开始，时序天然安全。
"""
...
def register_user_tool_message_types(types: List[Type[CustomAgentMessage]]) -> None:
    """注册一批消息类（按 ``role`` 判别）。
...
def _register_framework_types() -> None:
    ...
    """
    register_user_tool_message_types([OpaqueUserToolMessage])
