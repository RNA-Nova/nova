...
def register(registry: MethodRegistry, state: ServerState) -> None:
    def _settings_manager(params: Any) -> Any:
    ...
    async def getSettings(params: shapes.GetSettingsParams) -> shapes.GetSettingsResult:
        """合并生效配置（None 字段省略，前端按各自默认值兜底）。"""
        return shapes.GetSettingsResult(
            settings=_dump_effective(_settings_manager(params))
        )

    async def updateSettings(
        params: shapes.UpdateSettingsParams,
    ) -> shapes.UpdateSettingsResult:
        ...
        return shapes.UpdateSettingsResult(
            success=True, settings=_dump_effective(manager)
        )
    ...
    async def excludeResource(
        params: shapes.SetResourceExclusionParams,
    ) -> shapes.SetResourceExclusionResult:
        ...
        return shapes.SetResourceExclusionResult(
            success=True, patterns=getattr(manager, get_name)()
        )
