...
- ``NovaWireItem``（item 联合——框架变体 + ``CustomItem`` 包级兜底，归约层线上载体）；
...
CONTRACT_VERSION_MINOR = 0  # item 契约即首个公开契约（未发布——此前 1.x 计数作废重置）
...
class _Walker:
    ...
    def map(self, annotation: Any) -> Tuple[str, Dict[str, Any]]:
        ...
        # item 槽位特判：注解为 NovaItem 基类（通知的 item 载荷）或
        ...
        if annotation is NovaItem or annotation == Union[FrameworkItem, CustomItem, NovaItem]:
            ...
            for member in get_args(NovaWireItem):
                ...
                if is_dataclass(member) or _is_pydantic_model(member):
                    ...
                    member_refs.append({"$ref": f"#/$defs/{_ts_name(member)}"})
            self.reg.schema_defs["NovaWireItem"] = {"anyOf": member_refs}
            return "NovaWireItem", {"$ref": "#/$defs/NovaWireItem"}
    ...
    def _named_model(
        ...
        try:
            ...
            # extra="allow" 的模型（如 CustomItem）：命名字段之外透传任意键——
            # 线上确实如此（pydantic 保留额外字段并随 dump 落线），契约如实表达
            extra_allow = _is_pydantic_model(py_type) and (
                py_type.model_config.get("extra") == "allow"
            )
            if extra_allow:
                ts_lines.append("  [key: string]: unknown;")
            body = "\n".join(ts_lines)
            ...
            self.reg.schema_defs[name] = {
                ...
                "required": required,
                "additionalProperties": extra_allow,
            }
            return name, {"$ref": f"#/$defs/{name}"}
        finally:
...
def build_artifacts() -> Tuple[Dict[str, Any], str]:
    ...
    event_members = list(walker.add_union_root(AgentSessionEvent))
    # item 三态通知是纯线上词汇（reducer 产物，不上内部总线、不在
    # AgentSessionEvent 联合）——手动并入信封变体
    for notification in (
        ItemStartedNotification,
        ItemDeltaNotification,
        ItemCompletedNotification,
    ):
        walker._named_model(notification)
        event_members.append(notification)
    entry_members = walker.add_union_root(SessionEntry)
    item_members = walker.add_union_root(NovaWireItem)
    ...
    item_union = " | ".join(item_names)
    ts_parts.append(f"export type NovaWireItem = {item_union};\n")
    ...
    schema: Dict[str, Any] = {
        ...
        "novaItem": {"oneOf": [{"$ref": f"#/$defs/{name}"} for name in item_names]},
