# nova-client（含内置 TUI 宿主 modes/tui）设计文档（v3.1）

> 状态：**设计定案（v3.1，白纸重设计 + 扩展面补全）**。
> 总目标（不变）：**nova-client 是 pi 同等能力的 TS 产品运行时——智能体语义
...
| 其余一切：呈现推导 / TS 扩展 runtime / 渲染器 / 部件 / 主题 / 键位 / 命令 | **nova-client**（含内置 TUI 宿主 `modes/tui`） |
...
`nova-client/modes/tui/*` 子路径导出共享（jiti 别名 + 原生 ESM 缓存，
...
└── websocket.ts    【M3】远程宿主：WS **客户端**（WS 服务器/扇出已归 Python
                    rpc，2026-08 翻案——见 nova_architecture_2.0.md 文首修订）
```

`RuntimeHost` 是前端面对的唯一接口：

- `subscribe(model)`：渲染模型（item 清单镜像 + slot 产出 + 区域状态）变更订阅；
- `invoke(command, params)`：统一命令表调用；
- `onUIRequest/sendUIResponse`：反向原语转交；
- 客户端可以任何语言——**类型经 `nova-wire.schema.json` codegen**（语言中立）。
  归约归服务器（server/reduction——schema 生成不了状态机，也不该每客户端
  各写一遍）；本层只做 apply（item 三帧）与视图派生。

---

## 12. mirror：会话镜像（归约归服务器后的 apply 容器形态）

```
mirror/
├── mapping.ts    item 三帧应用（started 追加/delta 合并/completed 替换，
│                 delta 合并与后端同规则：text/output 白名单追加其余替换）
│                 + 域通知推导（工作状态/快照直写/本地通知）
│                 + deriveEntries/itemToEntry 视图派生（纯函数）
├── store.ts      sync（快照 + item 清单）+ apply（item 帧/域通知增量）+
│                 四事件增量直写（model_changed 等 payload 即事实）+ addNotice
└── types.ts      呈现自有词汇（视图条目/状态/通知）+ SessionSnapshot
                  （✅ R8：re-export 生成类型；item 类型同源 gen.ts）
```

mirror 经 bus 特权订阅接收事件（按序、恰好一次、先于观察者），并发布派生
便利事件（``session:synced`` / ``turn:started`` / ``turn:ended``）。

**归约归服务器**（server-item-layer 设计落地）：旧版的事件→呈现归约状态机
（流式槽/工具卡片配对/中断收尾/包消息映射）全部上移 server/reduction——
实时与恢复同形（金标测试在 server 侧），本层不再有"从消息重建内容"的逻辑。
