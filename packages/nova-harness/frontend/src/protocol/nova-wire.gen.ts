...
export const NOVA_CONTRACT_MINOR = 0;
...
export interface AgentMessageItem {
  id: string;
  type: "agentMessage";
  status: "pending" | "running" | "done" | "failed" | "declined" | "cancelled" | null;
  source: string | null;
  ts: number;
  text: string;
  error: string | null;
...
export type NovaEventEnvelope =
  ...
  | { type: "item_started"; data: ItemStartedNotification; seq: number; ts: number; sessionId: string | null }
  | { type: "item_delta"; data: ItemDeltaNotification; seq: number; ts: number; sessionId: string | null }
  | { type: "item_completed"; data: ItemCompletedNotification; seq: number; ts: number; sessionId: string | null };
...
export type NovaWireItem = UserMessageItem | AgentMessageItem | ThinkingItem | ToolCallItem | CompactionItem | BranchSummaryItem | CustomItem;
...
export interface NovaWireMethodMap {
  ...
  "compact": { params: CompactParams; result: CompactionResult };
