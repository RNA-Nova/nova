...
import type { ContentBlock, NovaUIRuntime, ToolCallCard } from 'nova-client';
...
export class ToolCardView extends Container {
  ...
  private maybeComputePreview(): void {
    if (this.card.status !== 'streaming' || this.card.argsComplete !== true) return;
    const compute = this.runtime.slots.resolveToolPreview(this.card.toolName);
    if (!compute) return;

    const argsKey = JSON.stringify(this.card.args);
    ...
    void compute(this.card.args, cwd)
      .then((data) => {
  ...
  private rebuild(): void {
    ...
    // （消掉"每条 transcript 事件重建所有卡片"：流式期间每 token 一次的
    // 无条件 rebuild 在此收敛为仅在真实变更时发生）。
    const fingerprint: readonly unknown[] = [
      this.card.status,
      this.card.argsComplete,
      this.card.args,
      this.card.partial,
      this.card.result,
      this.previewData,
    ...
    const summary = argsSummary(this.card);
    headerBox.addChild(
      new Text(
        colors.toolTitle(this.card.toolName) +
          (summary ? colors.dim(` ${summary}`) : '') +
          (this.card.status === 'streaming' && !this.card.argsComplete
            ? colors.dim(' …')
            : ''),
        0,
        0,
    ...
    // 执行前预览：streaming + argsComplete 时发起（可能在结果到达前完成注入）
    this.maybeComputePreview();

    const renderer = this.runtime.slots.resolveToolRenderer(this.card.toolName);
    const output = renderer?.({
      toolName: this.card.toolName,
      args: this.card.args,
      status: this.card.status,
      argsComplete: this.card.argsComplete,
      preview: this.previewData,
      partial: this.card.partial,
