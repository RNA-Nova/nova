...
import type { NovaUIRuntime, UINotice, UIRequest } from 'nova-client';
import type { NovaOverlayOptions } from 'nova-client';
import {
  Box,
  Container,
  Editor,
...
export class DialogController {
  ...
  /** 活动对话框的原语类型（能力消失自答的判定依据——dialog:* 槽位被卸时）。 */
  private activeComponent: string | undefined;
  ...
  constructor(
    ...
    /** host:openUrl 的开器（默认本机 spawn——测试注入防真开浏览器）。 */
    ...
    // 自动按 cancelled 应答并关框（能力变化前端先知，后端无从得知 slot 没了）
    this.runtime.onSlotsReplaced(() => this.dismissIfCapabilityLost());
  }
  ...
  /**
   * host:openUrl（执行调用宿主原语）：本机打开链接，即发即答。
   * 尽力而为——打开失败的回执由授权等待框里的 URL 兜底（显示是主通道）。
   */
  private handleOpenUrl(req: UIRequest): void {
    ...
    try {
      this.openUrlFn(url);
      this.runtime.sendUIResponse(req.id, { opened: true });
    } catch {
      this.runtime.sendUIResponse(req.id, { opened: false });
    }
  }
  ...
  private showCustomDialog(req: UIRequest): void {
    ...
    this.swap(req.id, component as Container, component as Component, req.component);
  }
  ...
  private showSelect(req: UIRequest): void {
    ...
    this.swap(req.id, selector, selector, req.component);
  }
  ...
  private showConfirm(req: UIRequest): void {
    ...
    this.swap(req.id, dialog, list, req.component);
  }
  ...
  private showInput(req: UIRequest): void {
    ...
    this.swap(req.id, dialog, input, req.component);
  }
  ...
  private showForm(req: UIRequest): void {
    ...
    this.swap(req.id, dialog, dialog, req.component);
  }
  ...
  /** 能力消失自答：slots 替换后活动弹窗的 dialog:* 槽位被卸——自动按
   * cancelled 应答并关框（该请求后端的 await 再无应答者）。 */
  private dismissIfCapabilityLost(): void {
    if (this.activeId === undefined || this.activeComponent === undefined) return;
    if (!this.activeComponent.startsWith('dialog:')) return;
    const factory = this.runtime.slots.resolve(this.activeComponent);
    if (typeof factory === 'function') return; // 槽位仍在
