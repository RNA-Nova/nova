...
export class NovaUIRuntime implements RuntimeHost {
  ...
  /** 全量同步：原子快照（syncSession：状态 + item 分页 + 事件高水位）→ mirror。
   * 启动与 session_replaced 后调用。条目图（entries）不进镜像——树导航
   * 按需经 getSessionTree/getSessionEntries 拉取。 */
  async syncFromBackend(): Promise<void> {
    const pageSize = 500;
    let offset = 0;
    let snapshot: SessionSnapshot | undefined;
    let items: NovaWireItem[] = [];
    // 逐页翻完：快照与水位每页刷新——末页三者（state/items/eventSeq）
    ...
    for (;;) {
      ...
      items = items.concat(page.items as NovaWireItem[]);
      if (page.items.length === 0 || offset + page.items.length >= page.totalItems)
        break;
      offset += page.items.length;
    }
    if (snapshot === undefined) return;
    this.store.sync(snapshot, items);
  }
  ...
  async clearQueue(): Promise<WireResult<'clearQueue'>> {
    return this.client.call('clearQueue', {});
  }
   ...
   */
  promptCancellable(text: string): {
    promise: Promise<WireResult<'prompt'>>;
    cancel: () => void;
  } {
  ...
  async updateSettings(settings: Record<string, unknown>): Promise<boolean> {
    ...
    if (result.success) await this.refreshSnapshot();
    return result.success;
  }
