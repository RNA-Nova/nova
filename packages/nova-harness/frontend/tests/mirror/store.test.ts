...
describe('MirrorStore', () => {
  it('sync 重建快照与 item 清单，并发布 session:synced', () => {
    ...
    store.sync(makeSnapshot(), [
      { id: 'e1', type: 'userMessage', status: null, source: 'user', ts: 1, content: [{ type: 'text', text: 'hi', textSignature: null }] },
      { id: 'e2', type: 'agentMessage', status: 'done', source: 'agent', ts: 2, text: 'yo', error: null },
    ]);
  ...
  it('status 迁移各 emit 一次 status；其余事件不 emit', () => {
    ...
    areas.length = 0;
    store.apply(ev('item_started', { item: { id: 'u1', type: 'userMessage', status: null, source: 'user', ts: 1, content: [] } }));
    store.apply(ev('model_changed', { model: { provider: 'openai', id: 'gpt-5' }, previousModel: null, source: 'user' }));
    ...
    store.apply(ev('item_delta', { id: 'nope', delta: {} })); // 未见 started 的 delta——无变更
    assert.deepEqual(
      areas.filter((area) => area === 'status'),
      ['status', 'status'],
    );
  ...
  it('mirror 经 bus 特权通道接收 item 帧（transcript 增量）', () => {
    const bus = new NovaBus();
    const store = new MirrorStore(bus);
    store.sync(makeSnapshot(), []);
    bus.publish(
      ev('item_started', {
        item: { id: 'i1', type: 'userMessage', status: null, source: 'user', ts: 1, content: [{ type: 'text', text: 'hello' }] },
      }),
    );
