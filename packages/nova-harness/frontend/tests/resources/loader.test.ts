...
export default function render(input: { toolName: string }) {
  return [{ kind: 'markdown', text: 'rendered:' + input.item.tool }];
}
`;
...
describe('loadUIAssets', () => {
  it('jiti 加载 .ts 渲染器并注册到 tool:<name> slot', async () => {
    ...
    assert.ok(renderer);
    const blocks = renderer({ item: { tool: 'bash', status: 'done' } }) as NovaBlock[];
...
`;
    ...
    const blocks = renderer?.({
      item: { tool: 'bash', status: 'done', result: { details: { command: 'ls -la' } } },
    });
