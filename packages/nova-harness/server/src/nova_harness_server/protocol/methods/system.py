...
def register(registry: MethodRegistry, state: ServerState) -> None:
    async def getCommands(params: shapes.EmptyParams) -> shapes.GetCommandsResult:
        if state.runtime is None:
            return shapes.GetCommandsResult(commands=[])
        ...
        return shapes.GetCommandsResult(commands=commands)

    async def getShortcuts(params: shapes.EmptyParams) -> shapes.GetShortcutsResult:
        """扩展注册的快捷键目录（键名/描述/来源）。
        ...
        """
        if state.runtime is None:
            return shapes.GetShortcutsResult(shortcuts=[])
        runner = state.runtime.session.extension_runner
        if runner is None:
            return shapes.GetShortcutsResult(shortcuts=[])
        return shapes.GetShortcutsResult(
            shortcuts=[
                {
    ...
    async def invokeShortcut(params: shapes.InvokeShortcutParams) -> shapes.OkResult:
        ...
        runner = state.runtime.session.extension_runner
        if runner is None:
            # 业务失败（前端按 success=False 呈现）——非协议错误
            return shapes.OkResult(success=False)
        handled = await runner.invoke_shortcut(key)
        return shapes.OkResult(success=handled)

    async def getExtensionFlags(
        params: shapes.EmptyParams,
    ) -> shapes.GetExtensionFlagsResult:
        """扩展注册的 CLI flags（定义 + 当前值）。"""
        if state.runtime is None:
            return shapes.GetExtensionFlagsResult(flags=[])
        runner = state.runtime.session.extension_runner
        if runner is None:
            return shapes.GetExtensionFlagsResult(flags=[])
        ...
        return shapes.GetExtensionFlagsResult(flags=flags)

    async def setExtensionFlag(
        params: shapes.SetExtensionFlagParams,
    ) -> shapes.SetExtensionFlagResult:
        ...
        # 入参经模型校验后 value 缺省即 None（模型默认值），flags[name].default
        # 兜底在旧 dict 归一化路径下已是死分支，语义不变
        value = params.value
        runner.set_flag_value(name, value)
        return shapes.SetExtensionFlagResult(success=True, name=name, value=value)

    async def cancelRequest(
        params: shapes.CancelRequestParams,
    ) -> shapes.CancelRequestResult:
        ...
        if task is None or task.done():
            return shapes.CancelRequestResult(success=True, cancelled=False)
        task.cancel()
        return shapes.CancelRequestResult(success=True, cancelled=True)
