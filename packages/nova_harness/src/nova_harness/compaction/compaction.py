...
from nova_ai import AssistantMessage, Model, Usage, complete_simple, Context, SimpleStreamOptions
    ...
    if prev_compaction_index >= 0:
        ...
        if isinstance(prev_compaction, CompactionEntry):
            if not prev_compaction.from_hook and prev_compaction.details:
                ...
                if hasattr(details, "read_files") and isinstance(details.read_files, list):
                    for f in details.read_files:
                        file_ops.read.add(f)
                if hasattr(details, "modified_files") and isinstance(details.modified_files, list):
...
def find_cut_point(
    ...
    is_user_message = (
        cut_entry.type == "message" and cut_entry.message.role == "user"
    )
    turn_start_index = (
...
async def generate_summary(
    ...
    """
    max_tokens = int(0.8 * reserve_tokens)  # Math.floor(0.8 * reserveTokens)
...
async def compact(
    ...
    if is_split_turn and turn_prefix_messages:
        ...
        turn_prefix_promise = _generate_turn_prefix_summary(
            turn_prefix_messages, model, settings.reserve_tokens, api_key, signal
        )
    ...
    return CompactionResult(
        ...
        first_kept_entry_id=first_kept_entry_id,
        tokens_before=tokens_before,
        details=CompactionDetails(read_files=list(read_files), modified_files=(modified_files)),
    )
...
async def _generate_turn_prefix_summary(
    ...
    """
    max_tokens = int(0.5 * reserve_tokens)  # Math.floor(0.5 * reserveTokens)
