...
from .remote_tool import RemoteTool  # 导入RemoteTool基类
...
class RemoteSkillTool(RemoteTool):
    ...
    async def execute(self, tool_call_id: str, params: dict,
                     signal: Optional[AbortSignal] = None, on_update=None) -> AgentToolResult:
        """执行技能工具操作"""
        ...
        try:
            ...
            if action == "list_categories":
                # 远程调用list_categories
                result = computex.list_categories()
                ...
                # 远程调用list_skills
                cat_info = computex.get_category_description(category)
                skills = computex.list_skills_by_category(category)
                ...
                skill_info = computex.get_skill_info(skill_name)
                if not skill_info:
                    raise ValueError(f"技能不存在: {skill_name}")
                
                directory_tree = computex.get_directory_tree(skill_name)
                skill_content = computex.get_skill_content(skill_name)
                ...
                # 远程调用read_file
                file_data = computex.read_file(skill_name, filename)
