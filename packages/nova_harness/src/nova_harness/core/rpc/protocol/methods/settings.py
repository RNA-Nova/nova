...
# 资源管控类键：变更后会话需重解析（注册表/系统提示词/资源全量重建）；
...
# 资源管控意图级方法的（读取, 写入）访问器对（名字级：tools / user_tools）
_PATTERN_ACCESSORS = {
...
def register(registry: MethodRegistry, state: ServerState) -> None:
    def _settings_manager(cwd: Optional[str]) -> Any:
    ...
    async def getSettings(params: _sh.GetSettingsParams) -> _sh.GetSettingsResult:
        """合并生效配置（None 字段省略，前端按各自默认值兜底）。"""
        return _sh.GetSettingsResult(
            settings=_dump_effective(_settings_manager(params.cwd))
        )

    async def updateSettings(
        params: _sh.UpdateSettingsParams,
    ) -> _sh.UpdateSettingsResult:
        """全局层部分更新：``{"settings": {...}}``，返回更新后的生效配置。"""
        manager = _settings_manager(params.cwd)
        ...
        if normalized & _RESOURCE_KEYS and state.runtime is not None:
            await state.runtime.session.reload()
        return _sh.UpdateSettingsResult(ok=True, settings=_dump_effective(manager))
    ...
    async def excludeResource(
        params: _sh.SetResourceExclusionParams,
    ) -> _sh.SetResourceExclusionResult:
        ...
        return _sh.SetResourceExclusionResult(
            ok=True, patterns=getattr(manager, get_name)()
        )

    async def includeResource(
        params: _sh.SetResourceExclusionParams,
    ) -> _sh.SetResourceExclusionResult:
    ...
    registry.register("getSettings", getSettings, domain=_D)
    registry.register("updateSettings", updateSettings, domain=_D)
    registry.register("excludeResource", excludeResource, domain=_D)
    registry.register("includeResource", includeResource, domain=_D)
