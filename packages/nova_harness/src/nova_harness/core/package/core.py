...
from nova_harness.core.package.deps import install_dependencies
...
class PackageManager:
    ...
    def install(
        ...
        Args:
            ...
            with_binaries: Attempt to install optional binaries (rg, fd).
        ...
        if resolved_kind == KIND_BUNDLE:
            return self._install_bundle(
                local_dir, source_obj, name, manifest, no_deps, with_binaries
            )
        return self._install_single(
            local_dir, source_obj, resolved_kind, name, manifest, no_deps, with_binaries
        )
    ...
    def uninstall(self, name: str, kind: str) -> bool:
    ...
    def _install_deps(self, package_dir: str, manifest, no_deps: bool) -> None:
        if no_deps:
            return
        ...
        if deps or has_requirements:
            install_dependencies(
                deps, requirements_path=requirements_path if has_requirements else None
            )
