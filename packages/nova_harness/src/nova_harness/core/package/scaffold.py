...
根据标准目录结构自动扫描 ``agents/``、``tools/``、``skills/``、``extensions/``、``prompts/``、``themes/``，
生成对应的资源清单并写入 ``pyproject.toml`` 的 ``[tool.nova]`` 段。
"""
...
def discover_entries(directory: str) -> Dict[str, List[str]]:
    """扫描目录，返回 agents/tools/skills/extensions/prompts/themes 的相对路径列表。"""
    ...
    for resource_type, subdir in [
        ...
        for entry in sorted(os.listdir(full)):
            ...
            if resource_type == "agents" and is_agent_dir(entry_path):
...
def _render_nova_section(
    ...
    lines.append(f"themes = {_toml_string_list(entries['themes'])}")
    lines.append(
