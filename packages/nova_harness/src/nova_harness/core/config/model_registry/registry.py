...
class ModelRegistry:
    ...
    def __init__(
        self, auth_storage: AuthStorage, models_json_path: Optional[str] = None
    ):
            ...
            for _, m in provider_models.items():
                ...
                if provider_override:
                    ...
                    if provider_override.compat is not None:
                        updates["compat"] = merge_compat(
                            model.compat, provider_override.compat
                        )
                    if provider_override.thinking_level_map is not None:
                        updates["thinking_level_map"] = (
                            provider_override.thinking_level_map
                        )
                    if updates:
    ...
    def _merge_custom_models(
        self, built_in: List[Model], custom: List[Model]
    ) -> List[Model]:
        """
        ...
        try:
            ...
            for provider_name, provider_config in config.providers.items():
                ...
                if (
                    ...
                    or provider_config.compat is not None
                    or provider_config.thinking_level_map is not None
                ):
                    overrides[provider_name] = ProviderOverride(
                        base_url=provider_config.base_url,
                        headers=provider_config.headers,
                        api_key=provider_config.api_key,
                        compat=provider_config.compat,
                        thinking_level_map=provider_config.thinking_level_map,
                    )

                # 存储 API key 用于 fallback resolver
                if provider_config.api_key:
                    self.custom_provider_api_keys[provider_name] = (
            ...
            return CustomModelsResult(
                models=self._parse_models(config, model_overrides),
                overrides=overrides,
    ...
    def _validate_config(self, config: ModelsConfig) -> None:
        ...
        for provider_name, provider_config in config.providers.items():
            ...
            for model_def in models:
                ...
                if (
                    model_def.context_window is not None
                    and model_def.context_window <= 0
                ):
                    raise ValueError(
    ...
    def _parse_models(
        self,
        config: ModelsConfig,
        model_overrides: Optional[Dict[str, Dict[str, ModelOverride]]] = None,
    ) -> List[Model]:
        """Parse custom model definitions from config and apply per-model overrides."""
        models: List[Model] = []
        ...
        for provider_name, provider_config in config.providers.items():
            ...
            per_model_overrides = model_overrides.get(provider_name, {})
            ...
            for model_def in model_defs:
                ...
                input_tuple: tuple[Literal["text", "image"], ...] = (
                    tuple(model_def.input) if model_def.input else ("text",)
                )
                ...
                model = Model(
                    ...
                    headers=headers,
                    compat=model_def.compat or provider_config.compat,
                    thinking_level_map=model_def.thinking_level_map
                    or provider_config.thinking_level_map,
                )

                # 应用 per-model 覆盖（对自定义模型同样生效）
                mo = per_model_overrides.get(model_def.id)
                if mo:
                    model = apply_model_override(model, mo)
    ...
    def register_provider(
        self, provider_name: str, config: ProviderConfigInput
    ) -> None:
        """
    ...
    def _apply_provider_config(
        ...
        if stream_simple:
            ...
            # 包装函数以适配 API 签名
            def stream_wrapper(
                model: Model, context: Context, options: Optional[SimpleStreamOptions]
            ):
                return stream_simple(model, context, options)

            class _DynamicApiAdapter:
                api = config.api

                def stream(self, model, context, options=None):
                    return stream_wrapper(model, context, options)

                def stream_simple(self, model, context, options=None):
                    return stream_simple(model, context, options)

            register_api_adapter(_DynamicApiAdapter())
            ...
            for model_def in config.models:
                ...
                self.models.append(
                    Model(
                        ...
                        max_tokens=model_def.max_tokens or 16384,
                        headers=headers,
                        compat=model_def.compat or config.compat,
                        thinking_level_map=model_def.thinking_level_map
                        or config.thinking_level_map,
                    )
                )

        elif (
            config.base_url
            or config.compat is not None
            or config.thinking_level_map is not None
        ):
            # 仅覆盖：更新现有模型的 base_url/headers/compat/thinking_level_map
            ...
            for m in self.models:
                if m.provider != provider_name:
                    ...
                    if config.compat is not None:
                        new_compat = merge_compat(m.compat, config.compat)
                    new_thinking_level_map = m.thinking_level_map
                    if config.thinking_level_map is not None:
                        new_thinking_level_map = config.thinking_level_map
                    new_models.append(
                        m.model_copy(
                            update={
                                ...
                                "headers": new_headers,
                                "compat": new_compat,
                                "thinking_level_map": new_thinking_level_map,
                            }
                        )
                    )
