...
def empty_custom_models_result(error: Optional[str] = None) -> CustomModelsResult:
    return CustomModelsResult(models=[], overrides={}, model_overrides={}, error=error)
...
def merge_compat(
    ...
    if isinstance(base_compat, OpenAICompletionsCompat) and isinstance(
        ...
        if override_compat.requires_mistral_tool_ids is not None:
            updates["requires_mistral_tool_ids"] = (
                override_compat.requires_mistral_tool_ids
            )
...
def apply_model_override(model: Model, override: ModelOverride) -> Model:
    ...
    # thinking_level_map 覆盖
    if override.thinking_level_map is not None:
        updates["thinking_level_map"] = override.thinking_level_map
