...
``SettingsManager``、``ModelRegistry``、``DefaultResourceLoader``）只要具备对应
成员即可被类型检查器接受。
"""
...
class AuthStorageProtocol(Protocol):
    ...
    def set_fallback_resolver(
        self, resolver: Callable[[str], Optional[str]]
    ) -> None: ...
    ...
    def set(self, provider: str, credential: Any) -> None: ...
...
class ModelRegistryProtocol(Protocol):
    """模型注册表能力，提供 API key 解析与动态 provider 注册。"""
...
class ToolsManagerProtocol(Protocol):
    ...
    def refresh(self, active_tool_names: Optional[List[str]] = None) -> None: ...
...
class ExtensionRunnerProtocol(Protocol):
    ...
    async def emit_session_shutdown(self, event: Any) -> Any: ...
    ...
    def set_ui_context(
        self, ui_context: Optional[Any] = None, mode: Any = "print"
    ) -> None: ...
...
class AgentSessionProtocol(Protocol):
    ...
    no_tools: Optional[Literal["all", "builtin"]]

    # 内部状态（控制器需要直接访问）
    _bash_abort_event: Optional[Any]
    _pending_bash_messages: List[Any]
