...
from nova_harness.core.types.protocols import ModelRegistryProtocol
...
class ExtensionRunner:
        ...
        for reg in list(runtime.pending_provider_registrations):
            ...
            target = provider_action or (
                lambda name, config=None: (
                    self.model_registry.unregister_provider(name)
                    if config is None and self.model_registry is not None
                    else (
                        self.model_registry.register_provider(name, config)
    ...
    def set_ui_context(
        self, ui_context: Optional[UIContext] = None, mode: ExtensionMode = "print"
    ) -> None:
    ...
    def get_shortcuts(
        self, resolved_keybindings: Optional[Dict[str, str]] = None
    ) -> Dict[str, ExtensionShortcut]:
        """收集扩展快捷键，检测与 builtin / 用户 keybinding 的冲突。"""
        from nova_harness.core.keybindings import check_extension_shortcuts

        self._shortcut_diagnostics = []
        shortcuts: Dict[str, ExtensionShortcut] = {}
        all_shortcuts: List[ExtensionShortcut] = []

        for ext in self.extensions:
            for key, shortcut in ext.shortcuts.items():
                normalized = key.lower()
                if normalized not in shortcuts:
        ...
        # 检测与 builtin / 用户 keybinding 的冲突
        for diag in check_extension_shortcuts(all_shortcuts, resolved_keybindings):
            self._shortcut_diagnostics.append(
                {
                    "type": diag.type,
                    "message": diag.message,
                    "path": diag.extension_path,
                }
            )
    ...
    def create_context(self, extension: Optional[Extension] = None) -> ExtensionContext:
        ...
        return ExtensionContext(
            ...
            get_model=context_actions.get_model,
            ...
            ui=self.ui_context or NoOpUIContext(),
            has_ui=self.has_ui(),
            mode=self.mode,
            cwd=self.cwd,
    ...
    async def emit(self, event: Any) -> Any:
        """通用事件分发。对 session_before_* 事件收集结果，支持 cancel 短路。"""
            ...
            for handler in handlers:
                try:
                    ...
                    if inspect.isawaitable(raw):
                        raw = await raw
                    results.append(raw)
                    # cancel 短路：session_before_* / before_agent_start 等事件
                    # 遇到显式 cancel=True 时立即停止
    ...
    async def emit_session_shutdown(
        self, event: SessionShutdownEvent
    ) -> SessionShutdownEventResult:
        """触发 session_shutdown 事件。"""
        return await self._emit_single_result(event, SessionShutdownEventResult)

    async def emit_project_trust(
        self,
        event: ProjectTrustEvent,
        context: ProjectTrustContext,
    ) -> Tuple[Optional[ProjectTrustEventResult], List[Dict[str, str]]]:
        """触发 project_trust 事件，让扩展参与项目信任决策。

        第一个明确返回 ``yes``/``no`` 的 handler 获胜；返回 ``undecided`` 的
        handler 会被跳过，继续询问后续扩展。所有扩展都未决定时返回
        ``(None, errors)``。
        """
            ...
            for handler in handlers:
                try:
                    ...
                    if isinstance(raw, dict):
                        trusted_value = raw.get("trusted")
                        remember = bool(raw.get("remember", False))
                    else:
                        trusted_value = getattr(raw, "trusted", None)
                        remember = bool(getattr(raw, "remember", False))

                    if trusted_value == "undecided":
                        continue

                    if trusted_value in (True, "yes"):
                        return ProjectTrustEventResult(
                            trusted="yes", remember=remember
                        ), errors
                    if trusted_value in (False, "no"):
                        return ProjectTrustEventResult(
                            trusted="no", remember=remember
                        ), errors
                except Exception as exc:
                    error = {
                        "extension_path": ext.path,
