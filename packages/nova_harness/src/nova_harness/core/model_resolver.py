...
from nova_agent import ThinkingLevel
...
def _is_valid_thinking_level(value: Optional[str]) -> bool:
    ...
    if value is None:
        return False
    return value in {level.value for level in ThinkingLevel}
...
def parse_model_pattern(
    ...
    *,
    ...
    if _is_valid_thinking_level(suffix):
        ...
        return ParsedModelResult(
            model=inner.model,
            thinking_level=inner.warning is None and ThinkingLevel(suffix) or None,
...
def resolve_model_scope(
    ...
    与 TS 对齐，只包含有鉴权的可用模型（``get_available``）。
    """
    ...
    *,
    ...
    if parsed.model is not None:
        if inferred_provider:
            ...
            if (
                raw_exact
                and hasattr(model_registry, "has_configured_auth")
                and not model_registry.has_configured_auth(parsed.model)
            ):
                authenticated = [
                    m for m in raw_exact if model_registry.has_configured_auth(m)
                ]
    ...
    if provider is not None:
        ...
        fallback_thinking: Optional[ThinkingLevel] = None
        last_colon = pattern.rfind(":")
        if last_colon != -1:
            suffix = pattern[last_colon + 1 :]
            if _is_valid_thinking_level(suffix):
                fallback_pattern = pattern[:last_colon]
                fallback_thinking = ThinkingLevel(suffix)
...
async def find_initial_model(
    ...
    *,
    ...
    4. settings 中的默认模型
    5. 任意一个有鉴权的可用模型（优先 ``DEFAULT_MODEL_PER_PROVIDER``）
    """
...
async def restore_model_from_session(
    *,
    ...
    if restored is not None:
        if hasattr(model_registry, "has_configured_auth"):
            has_auth = model_registry.has_configured_auth(restored)
        else:
...
def resolve_thinking_level(
    ...
    - 无 model 时返回 None（off）。
    - 按 model.thinking_level_map 降级到最高可用级别。
    """
