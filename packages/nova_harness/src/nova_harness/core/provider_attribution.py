...
from nova_harness.core.config.telemetry import is_install_telemetry_enabled
...
def _is_openrouter_model(model: Model) -> bool:
    return model.provider == "openrouter" or _matches_host(model.base_url, _OPENROUTER_HOST)
...
def _is_opencode_model(model: Model) -> bool:
    return (
        model.provider in ("opencode", "opencode-go")
        or _matches_host(model.base_url, _OPENCODE_HOST)
    )
...
def _get_session_headers(model: Model, session_id: Optional[str]) -> Optional[Dict[str, str]]:
