...
from nova_harness.core.types.messages import BashExecutionMessage, CustomMessage
...
class SessionManager:
    ...
    def __init__(
        self, cwd: str, session_dir: str, session_file: Optional[str], persist: bool
    ):
        self._cwd = cwd
    ...
    def _build_index(self) -> None:
        """构建索引，并按 leaf entry 规则恢复当前 leaf。"""
        self._by_id.clear()
        self._labels_by_id.clear()
        self._leaf_id = None
        ...
        for entry in self._file_entries:
            ...
            if entry.type == "leaf":
                # leaf entry 重定向当前 leaf 到 target_id
                self._leaf_id = entry.target_id
            else:
    ...
    def append_message(
        self, message: Union[Message, CustomMessage, BashExecutionMessage]
    ) -> str:
        """追加消息"""
    ...
    def append_thinking_level_change(
        self, thinking_level: Optional[ThinkingLevel] = None
    ) -> str:
        """追加思考级别变更"""
    ...
    def append_active_tools_change(self, active_tool_names: List[str]) -> str:
        """追加激活工具变更条目"""
    ...
    def set_leaf_id(self, target_id: Optional[str]) -> Optional[str]:
        """持久化 leaf 指针变更，写入一个 leaf entry。"""
        entry = LeafEntry(
            type="leaf",
            id=generate_id(set(self._by_id.keys())),
            parent_id=self._leaf_id,
    ...
    def open(cls, path: str, session_dir: Optional[str] = None) -> "SessionManager":
        ...
        dir_path = (
            ...
            else os.path.abspath(os.path.dirname(path))
        )
    ...
    @classmethod
    def fork_from(
        cls, source_path: str, target_cwd: str, session_dir: Optional[str] = None
    ) -> "SessionManager":
        """从源会话fork新会话"""
        source_entries = load_entries_from_file(source_path)
        if not source_entries:
            raise ValueError(
                f"Cannot fork: source session file is empty or invalid: {source_path}"
            )
        ...
        if not source_header:
            raise ValueError(
                f"Cannot fork: source session has no header: {source_path}"
            )
        ...
        with open(new_session_file, "w", encoding="utf-8") as f:
            f.write(json.dumps(new_header.model_dump(), ensure_ascii=False) + "\n")
