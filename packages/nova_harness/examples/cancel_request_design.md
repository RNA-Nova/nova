...
（`packages/nova-tui/src/runtime.ts:234-236`）薄转发到
`client.call('prompt', {text})`，得到**一个挂着的 Promise**：直到后端这次
调用完成才会 settle。

`client.call`（`packages/nova-tui/src/wire/client.ts:182-201`）的
...
后端 RPC 服务器（`packages/nova_harness/src/nova_harness/core/rpc/server.py`）
...
（`packages/nova_harness/src/nova_harness/core/agent_session/agent.py:1141-1145`）：
...
`core/rpc/protocol/ui_context.py:65-122`）的结构是：
...
**文件**：`core/rpc/protocol/methods/state.py`
...
**文件**：`core/rpc/server.py` 的 `_handle_raw`（当前 138-152 行）
...
**文件**：`core/rpc/protocol/methods/system.py`（传输层操作归 system 域，
与 initialize 等同域）
...
**文件**：`core/rpc/protocol/ui_context.py` 的 `request`（当前 111-122 行）
...
- `schema_export.py:56`：`CONTRACT_VERSION_MINOR` 1.0 → 1.1（加法语义：
  新方法，无破坏性变更）；
- 重跑导出：
  `python -m nova_harness.core.rpc.protocol.schema_export`（写
  `packages/nova-tui/protocol/nova-wire.schema.json` 与
...
**文件**：`packages/nova-tui/src/wire/client.ts`
...
**文件**：`packages/nova-tui/src/runtime.ts`（命令 API 区）
...
10. `nova-tui` npm test 全绿 + `nova-tui`/`nova-tui` tsc
    build 通过。
...
| 事实 | 位置 |
|---|---|
| server 一消息一 task，`_pending_tasks` 是无寻址能力的 set | `core/rpc/server.py:110-112, 49` |
| 服务器关闭时已批量 cancel task（取消语义已在用） | `core/rpc/server.py:115-118` |
| `_handle_raw` 现行结构（parse → ui 特判 → dispatch → 写应答） | `core/rpc/server.py:138-152` |
...
| `TransportUIContext.request`：signal 竞速有、CancelledError 无 | `core/rpc/protocol/ui_context.py:65-122` |
| 前端 `client.call` 无取消句柄；pending 按 id 配对；无条目静默丢弃 | `nova-tui/src/wire/client.ts:182-201, 225-234` |
| 反向通道三帧（ui/request、ui/notify、ui/cancel）路由齐全 | `nova-tui/src/wire/bridge.ts:90-124` |
