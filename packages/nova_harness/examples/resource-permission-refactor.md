...
> nova-tui 325 + 前端 178 全绿；PTY 矩阵 45/45；全栈冒烟通过。
...
- **目录归并**：`packages/nova_harness/{backend,frontend}` 伞目录
  （现 nova_harness → backend，现 nova-tui → frontend；import 名不变；
...
- 前端 UI 资产的 settings 开关（nova-tui 管线另案）。
