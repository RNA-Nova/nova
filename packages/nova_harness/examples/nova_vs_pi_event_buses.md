...
| 项目 | 位置 | 核心实现 |
|---|---|---|
| Nova | `packages/nova-harness/backend/src/nova_harness/core/agent_session/agent.py:596` | `def _emit(self, event: Any)` |
| π | `packages/coding-agent/src/core/agent-session.ts:458` | `private _emit(event: AgentSessionEvent)` |

**Nova 的 AgentSessionEvent 定义**：

```python
# packages/nova-harness/backend/src/nova_harness/core/types/events/unions.py:73
...
Agent._emit
  └─> AgentSession 订阅
        ├─> Nova: EventController.handle() (packages/nova-harness/backend/src/nova_harness/core/agent_session/controllers/event.py)
...
| 项目 | 位置 | 核心实现 |
|---|---|---|
| Nova | `packages/nova-harness/backend/src/nova_harness/core/extensions/runner.py:476` | `async def emit(self, event: Any)` |
...
| 项目 | 位置 | 核心实现 |
|---|---|---|
| Nova | `packages/nova-harness/backend/src/nova_harness/core/extensions/event_bus.py` | `class ExtensionEventBus` |
...
**暴露方式**：
- Nova：`NovaExtensionAPI.events = event_bus`（`packages/nova-harness/backend/src/nova_harness/core/extensions/api.py:47`）
...
**关键缺口**：`packages/nova-harness/backend/src/nova_harness/core/ui/mapper.py:112` 的 `map_event` 函数处理了 `ToolExecutionStartEvent` 和 `ToolExecutionEndEvent`，但没有处理 `ToolExecutionUpdateEvent`。这导致 RPC/WebSocket 前端看不到 bash 等工具的流式输出。
...
```python
# packages/nova-harness/backend/src/nova_harness/core/server.py:73
...
```python
# packages/nova-harness/backend/src/nova_harness/core/ui/events.py
UIEvent = Union[
...
`packages/nova-harness/backend/src/nova_harness/core/ui/context.py`
