...
- **加载函数**：`resources/loaders/agent_config.py::load_agent_config_from_dir()`（由 `DefaultResourceLoader._reload_agents` 逐个调用，同名碰撞 first-wins）
- **最终形式**：
  ```python
...
- **内容包含**：
  ...
  - `tools`: `List[ToolInfo]`
  - `skills`: `List[str]`（包内 skill 裁剪名单：空=全放，非空=仅裁包内）
  - `extensions`: `List[str]`
...
- **消费方**：
  - `SystemPromptManager` 按当前 agent 的 `skills` 名单裁剪包内 skill 后（空名单=全放），注入 system prompt
...
- `get_skills()` → 按 agent `skills` 名单裁剪包内 skill 后注入（空名单=全放）
...
| tools | ✅ | ✅ | ✅ | |
| skills | ✅ | ✅ | ✅（包内可经名单裁剪） | `/skill:name` 已支持 |
