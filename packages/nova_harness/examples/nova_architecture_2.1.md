...
nova-wire.schema.json（语言中立快照）+ nova-wire.gen.ts（TS 判别联合）
   ...
   ▼
nova-tui：mapping/store/rpc-client 全部基于生成类型
```
...
**多后端 = 同一 Node 层可接入不同语言的 harness 实现**：nova-tui 是常量，
...
## 3. nova-tui（Node，TS）：呈现的一切

> 本节的完整施工图（定位/模块/机制/判决记录）见
> **`packages/nova-tui/docs/design.md`**；TUI 已作为内置宿主落位于
...
- **组件模型：声明式块词汇优先**——渲染器返回声明式描述而非框架组件：
  v1 = `diff / markdown / code / json / table` 五种（**已落地**：`nova-tui/src/presentation/blocks.ts` +
