    ...
    with patch("nova_harness.package.install.installer.install_dependencies"):
        with patch("nova_harness.package.install.installer.install_package"):
            with patch("nova_harness.package.install.installer.uninstall_package"):
                with patch("nova_harness.package.manager.uninstall_package") as mock:
                    yield mock
...
def test_reinstall_same_name_uninstalls_orphan_python_package(pm, tmp_path):
    ...
    with patch(
        "nova_harness.package.install.installer.uninstall_package"
    ) as mock_uninstall:
...
def test_install_extension_triggers_python_package_install(
    ...
    with patch("nova_harness.package.install.installer.install_package") as mock_pkg:
        meta = pm.install(str(pkg))
...
def test_no_deps_skips_dependency_install(pm, tmp_path):
    ...
    with patch("nova_harness.package.install.installer.install_dependencies") as mock:
...
def test_dependency_install_called(pm, tmp_path):
    ...
    with patch(
        "nova_harness.package.install.installer.check_dependency_conflicts"
    ) as mock:
        pm.install(str(pkg))
...
def test_install_editable_installs_deps_and_editable_package_for_tools(
    ...
    with patch(
        ...
        with patch(
            "nova_harness.package.install.installer.install_dependencies"
        ) as mock_deps:
            with patch(
                "nova_harness.package.install.installer.install_package"
            ) as mock_pkg:
                meta = pm.install(str(pkg), editable=True)
