"""
assembly 组装编排测试（原 tests/core/sdk/test_high_level.py）。
"""
...
def test_list_installed_agents(tmp_path: Path) -> None:
    """通过已安装包发现 agent 组合声明（``agents/<name>.yaml``，取文件名 stem）。"""
    pkg_dir = tmp_path / "nova-coding-agent"
    agents_dir = pkg_dir / "agents"
    agents_dir.mkdir(parents=True)
    (agents_dir / "coding.yaml").write_text(
        "description: Coding agent\n", encoding="utf-8"
    )
    # 非 yaml 条目与目录不产生 agent 名
    (agents_dir / "notes.md").write_text("not an agent\n", encoding="utf-8")
    (agents_dir / "empty").mkdir()

    with patch(
        "nova_harness.core.runtime_manager.assembly.PackageManager"
    ) as mock_pm_cls:
...
def _fake_metadata(pkg_dir: Path):
    ...
    from nova_harness.types.package.manifest import PackageMetadata
