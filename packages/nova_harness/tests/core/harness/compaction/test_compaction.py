...
def _bash(command: str, output: str) -> "FakeUserToolMessage":
    """构造一条包级用户工具消息（bashExecution 的最小替身）。"""
    return FakeUserToolMessage(
        text=f"Ran `{command}`\n{output}",
        timestamp=1700000000000,
    )


class FakeUserToolMessage(CustomAgentMessage):
    """测试用用户工具消息（实现 ContextInjectable 协议）。"""

    text: str = ""
    timestamp: int = 0
    exclude_from_context: bool = False
    role: Literal["fakeUserTool"] = "fakeUserTool"
...
def _stream_with_result(result: AssistantMessage) -> AssistantMessageEventStream:
    ...
    stream.push(DoneEvent(reason=result.stop_reason, message=result))
    return stream
...
def test_prepare_compaction_with_previous_compaction(session):
    ...
    a6 = session.append_message(_assistant("answer3" + "x" * 400))  # noqa: F841
...
def test_prepare_compaction_returns_none_when_nothing_to_summarize(session):
    """预算内全部保留时（两路消息皆空），不发摘要请求（对齐 TS 空摘要短路）。"""
    session.append_message(_user("first"))
    session.append_message(_assistant("answer1"))
    u2 = session.append_message(_user("second"))
    session.append_compaction("old summary", u2, 100)
    session.append_message(_user("third"))
    session.append_message(_assistant("answer3"))
...
def test_build_context_ignores_custom_entry(session):
    u1 = session.append_message(_user("first"))
    session.append_custom_entry("plugin_state", {"key": "value"})
    session.branch(u1)

    ctx = session.build_session_context()
    # custom entry 不参与 context
...
# -----------------------------------------------------------------------------
# Async compaction / generate_summary / complete_summarization
# -----------------------------------------------------------------------------
...
async def test_compact_non_split_turn(session, monkeypatch):
    ...
    monkeypatch.setattr(compaction_module, "complete_summarization", fake_complete)
...
async def testcomplete_summarization_with_sync_stream_fn():
    """complete_summarization 调用同步自定义 stream_fn。"""
    called = {}
    ...
    options = SimpleStreamOptions(api_key="key", max_tokens=10)

    result = await compaction_module.complete_summarization(
...
async def testcomplete_summarization_with_async_stream_fn():
    """complete_summarization 调用异步自定义 stream_fn。"""
    called = {}
