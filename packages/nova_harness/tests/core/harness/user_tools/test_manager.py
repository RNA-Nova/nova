...
async def test_controller_invoke_records_message_when_idle():
    ...
    # 工具进度经会话事件总线透出（user_tool 事件）+ record 的消息
    # 定稿事件（MessageStart/MessageEnd——前端卡片完结依赖）
    emitted_types = [event.type for event in session._emitted]
    assert emitted_types == ["user_tool", "message_start", "message_end"]
    event = session._emitted[0]
    assert event.tool == "fake"
    assert event.event == "progress"
    assert session._emitted[1].message is message
    assert session._emitted[2].message is message
