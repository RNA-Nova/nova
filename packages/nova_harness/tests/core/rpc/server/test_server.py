...
async def test_session_event_forwarded(transport_pair, echo_methods):
    ...
    # 哑管道直通：信封 {type, data}，事件字段原样在 data 里
    assert resp["params"]["type"] == "tool_execution_start"
    assert resp["params"]["data"]["toolName"] == "bash"
    assert resp["params"]["data"]["toolCallId"] == "tc-1"
...
async def test_rebind_session_resubscribed(transport_pair, echo_methods):
    ...
    assert resp["params"]["type"] == "tool_execution_start"
    assert resp["params"]["data"]["toolName"] == "edit"
...
async def test_cancel_request_end_to_end(transport_pair):
    ...
    assert resp_late["result"] == {"ok": True, "cancelled": False}
