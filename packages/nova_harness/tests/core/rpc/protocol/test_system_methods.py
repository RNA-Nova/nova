...
from nova_harness.core.rpc.protocol import JSONRPCError, build_request
from nova_harness.core.rpc.protocol.methods.state import ServerState
from nova_harness.core.rpc.protocol.methods.system import register
...
async def test_set_extension_flag_updates_value():
    ...
    assert result is not None and result.error is None
    assert result.result == {"ok": True, "name": "verbose", "value": True}
...
async def test_cancel_request_cancels_running_task():
    ...
    assert result is not None and result.error is None
    assert result.result == {"ok": True, "cancelled": True}
...
async def test_cancel_request_idempotent_for_unknown_or_done():
    ...
    assert result is not None and result.error is None
    assert result.result == {"ok": True, "cancelled": False}
