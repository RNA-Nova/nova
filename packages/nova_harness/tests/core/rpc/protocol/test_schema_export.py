...
from nova_harness.core.rpc.protocol import schema_export


def _repo_root() -> Path:
    # tests/core/rpc/protocol/test_schema_export.py → 仓库根
...
def test_committed_artifacts_are_fresh():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "nova_harness.core.rpc.protocol.schema_export",
            "--check",
        ],
        capture_output=True,
        text=True,
...
def test_envelope_covers_bus2_events():
    ...
    # Bus 2 当前 22 个事件 type（与 AgentSessionEvent union 一一对应）
    expected = {
        "agent_start",
        "agent_end",
        "agent_settled",
