...
def test_collect_theme_entries(tmp_path) -> None:
    theme = tmp_path / "dark.json"
    theme.write_text("{}")
    assert collect_theme_entries(str(tmp_path)) == [str(theme.resolve())]


def test_collect_theme_entries_respects_gitignore(tmp_path) -> None:
    """theme 发现应尊重 .gitignore 中的排除规则。"""
    (tmp_path / "dark.json").write_text("{}", encoding="utf-8")
    (tmp_path / "ignored.json").write_text("{}", encoding="utf-8")
    (tmp_path / ".gitignore").write_text("ignored.json\n", encoding="utf-8")

    assert collect_theme_entries(str(tmp_path)) == [
        str((tmp_path / "dark.json").resolve())
    ]


def test_collect_theme_entries_recursive(tmp_path) -> None:
    """theme 发现应递归子目录。"""
    nested = tmp_path / "nested"
    nested.mkdir()
    (nested / "dark.json").write_text("{}", encoding="utf-8")

    themes = collect_theme_entries(str(tmp_path))
    assert str((nested / "dark.json").resolve()) in themes
...
def test_collect_tool_entries_respects_gitignore(tmp_path) -> None:
    ...
    ignored.mkdir()
    (ignored / "schema.json").write_text("{}")
...
def test_collect_agent_entries_respects_gitignore(tmp_path) -> None:
    """agent 发现应尊重 .gitignore 中的排除规则。"""
    from nova_harness.core.package.discovery import collect_agent_entries

    agent_dir = tmp_path / "coding"
    agent_dir.mkdir()
    (agent_dir / "description.md").write_text("coding agent")

    ignored = tmp_path / "ignored"
    ignored.mkdir()
    (ignored / "description.md").write_text("ignored agent")

    (tmp_path / ".gitignore").write_text("ignored/\n", encoding="utf-8")

    assert collect_agent_entries(str(tmp_path)) == [str(agent_dir.resolve())]
...
def test_collect_explicit_respects_gitignore_for_files(tmp_path):
    ...
    result = _collect_explicit(["./keep.md", "./ignored.md"], tmp_path)
...
def test_collect_explicit_glob_respects_gitignore(tmp_path):
    ...
    result = _collect_explicit(["./*.md"], tmp_path)
...
def test_collect_explicit_with_override_patterns(tmp_path):
    ...
    result = _collect_explicit(items, tmp_path)
...
def test_collect_explicit_directory_recursive_for_extensions(tmp_path):
    ...
    result = _collect_explicit(["./*"], tmp_path, resource_type=ResourceType.THEMES)
    assert str((theme_dir / "dark.json").resolve()) in result
...
def test_collect_ancestor_agents_skills_dirs(tmp_path):
    ...
    try:
        dirs = collect_ancestor_agents_skills_dirs(
            str(child), stop_at_git_root=True
        )
