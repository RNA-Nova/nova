...
class RpcServer:
    ...
    # 内容事件不上线（归约层消费后产 item 帧——客户端只吃归约成品）；
    # 域通知与生命周期事件直通
    ...
    def __init__(
        ...
        *,
        ...
        self._reducer: Optional[SessionReducer] = None
    ...
    def _subscribe_session(self, session: Any) -> None:
        ...
        if self._unsubscribe_session is not None:
            self._unsubscribe_session()
        # 归约器随会话挂摘：内容事件在此归约为 item 通知（类型化帧），
        # 域通知直通广播；item_emission 是纯内部信封，只喂归约器不上线
        reducer = SessionReducer(emit=self._emit_reduced)
        self._reducer = reducer
        self._state.item_reducer = reducer

        def listener(event: Any, signal: Any = None) -> None:
            event_type = getattr(event, "type", None)
            if event_type == ITEM_EMISSION:
                reducer.handle_event(event)
                return
            payload = serialize_event(event)
            if payload is not None and event_type not in self._CONTENT_EVENT_TYPES:
                self.broadcast_event(payload)
            reducer.handle_event(event)
            # 作用域仲裁的挂接点（codex cancel_requests_for_thread 对位）：
            # run 终结（agent_end）清扫该 run 的挂起 UI 请求；
            # 会话替换（session_replaced）清扫 run/session 两级（global 不动）
            cancel_scope = getattr(self.ui, "cancel_scope", None)
            if cancel_scope is None:
                return
            if payload is None:
                return
            if payload.get("type") == "agent_end":
                run_id = (payload.get("data") or {}).get("run_id")
                if run_id:
                    cancel_scope(f"run:{run_id}")
            elif payload.get("type") == "session_replaced":
                cancel_all = getattr(self.ui, "cancel_session_scopes", None)
                if callable(cancel_all):
                    cancel_all()

        self._unsubscribe_session = session.subscribe(listener)

    def _emit_reduced(self, notification: Any) -> None:
        """归约产物的广播出口（item 通知帧——与域通知同一信封纪律）。"""
        payload = serialize_event(notification)
