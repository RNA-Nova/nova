...
```
src/nova_ai/
├── __init__.py              # 本包门面（行为层 / gateway / providers / stream_options / utils；
│                            #   跨组件类型词汇一律住 nova_protocol，本包不再导出）
├── stream_options.py        # StreamOptions、SimpleStreamOptions、ThinkingBudgets、ProviderResponse（dataclass）
├── streaming.py             # EventStream、AssistantMessageEventStream（异步迭代器 + result Future）
├── model_catalog.py         # flatten_model_catalog（数据分片 → Model 拍平）
├── models_generated.py      # 生成的模型目录聚合器
├── gateway/                 # Models 网关层（对齐 TS models.ts + models-store.ts）
│   ├── models.py            # Models 集合 + create_models（auth 网关 + provider 注册表 + 刷新世代）
│   ├── provider.py          # Provider / _DynamicProvider / create_provider（模型目录宿主 + 协议路由）
│   ├── streams.py           # lazy_stream（ProviderStreams 契约原语：同步签名 + 异步装配 + 错误即流终态）
│   └── store.py             # ModelsStore / ProviderModelsStore 协议与 InMemory 实现
│                            #   （落盘 schema ModelsStoreEntry 住 nova_protocol）
├── api_impls/               # API 协议实现
│   ├── _shared/             # 实现层共享件（transform_messages / simple_options / retry /
│   │                        #   prompt_cache / copilot_headers / user_agent）
│   └── openai_completions/  # OpenAI Completions（当前唯一完整实现；模块即满足 ProviderStreams）
│                            #   compat / messages / client / params / reasoning / _stream 分件
├── providers/               # 内置 provider 工厂与静态模型目录（每个 provider 一个子目录）
│   ├── all.py               # builtin_providers() / builtin_models()
│   ├── catalog.py           # 目录数据源与修正层（构建期种子与运行时物化共用单源）
│   ├── remote_catalog.py    # with_remote_catalog（远程目录 overlay + 新鲜度竞速）
...
├── auth/                    # 鉴权行为层（类型词汇住 nova_protocol.auth）
│   ├── resolve.py           # resolve_provider_auth + ModelsError + AuthResolutionOverrides
...
└── utils/                   # 跨层通用工具函数（实现层共享件归 api_impls/_shared）
    ...
    ├── json_parser.py       # 流式 JSON 片段解析（json-repair 封装 + 增量解析器）
    ├── surrogate.py         # Unicode 代理项清理
    ├── overflow.py          # 上下文溢出检测（多厂商错误模式）
    ├── hash.py              # short_hash（与 TS 位级对齐的确定性短哈希）
    └── abort.py             # race_with_abort / any_signal / operation_signal
```
...
3. **OpenAI Completions 实现**  
   当前唯一完整的协议实现位于 `api_impls/openai_completions/`，使用官方 `openai.AsyncOpenAI` 客户端：
...
4. **消息转换与跨模型兼容**  
   `api_impls/_shared/transform_messages.py` 的 `transform_messages()` 两趟处理：思考块保留/转文本、跨模型删除 `thought_signature` 与工具调用 ID 规范化；为孤立工具调用插入合成 `ToolResultMessage`；跳过 `ERROR` / `ABORTED` 的不完整助手消息。重建消息一律用 `model_copy(update=...)` 保留未修改字段（如 `diagnostics`、`added_tool_names`）。

5. **序列化层约定（选型结果速查）**  
   选型依据遵循根 `AGENTS.md` 的"数据建模"决策顺序（可变性 → 序列化 → 校验价值 → 禁用项），住所规则见 `docs/data-modeling.md`。本包现有归类：
   - **Pydantic（`NovaBaseModel`）**：需要 JSON parse/dump 的类型——全部住 `nova_protocol` 枢纽：`Model`/`Usage`（models.json、会话 JSONL）、messages/content（会话 + RPC）、events（RPC 事件流）、compat（models.json）、`ApiKeyCredential`/`OAuthCredential`（auth.json，`OAuthCredential` 用 `extra="allow"` 保留扩展字段）、`ModelsStoreEntry`（models-store.json）。
   - **dataclass**：纯代码构造、跨包传递的运行时容器，持 `Callable` 或服务实例——`StreamOptions` 家族（构造签名即契约，传错字段直接 `TypeError`）、`Provider`、`RefreshModelsContext`、`ApiKeyAuth`/`OAuthAuth`/`ProviderAuth`、`AuthResult`/`AuthCheck` 等（不可变值对象一律 `frozen=True, kw_only=True`）。
   - **TypedDict / Protocol**：`ModelAuth` / `ApiKeyAuthInput`（dict 形状透传——声明不校验）、`AuthContext`/`CredentialStore`/`AuthInteraction`/`ProviderStreams`/`ModelsStore`。
...
4. **兼容检测的潜在风险**  
   `api_impls/openai_completions/compat.py` 按 `base_url` 子串推断兼容标志（`volces.com`、`api.x.ai` 等）。URL 被恶意构造可能诱导不安全的参数回退；修改自动检测逻辑时保持谨慎。
...
- **新增类型先想选址与选型**：跨组件边界的纯数据词汇进 `nova_protocol` 枢纽（消费方一律 `from nova_protocol import X`，本包不中转再导出）；组件内部类型留本包。表示选型按根 `AGENTS.md` 的"数据建模"决策顺序（可变性 → 序列化 → 校验价值 → 禁用项）选定 `NovaBaseModel` / dataclass / TypedDict / Protocol。
- **新增 provider**：
...
- **修改消息类型需谨慎**：`nova_protocol` 的 `messages.py` 与 `content.py` 是整个 monorepo 的公共契约。
