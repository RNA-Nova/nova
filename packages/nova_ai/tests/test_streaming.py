...
class TestStreamingEvents:
    ...
    def test_text_delta_event(self):
        event = TextDeltaEvent(
            content_index=0, delta="hello", partial=AssistantMessage(content=[]),
        )
    ...
    def test_text_end_event(self):
        event = TextEndEvent(
            content_index=0, content="hello", partial=AssistantMessage(content=[]),
        )
        assert event.type == "text_end"
        assert event.content == "hello"

    def test_thinking_start_event(self):
        event = ThinkingStartEvent(content_index=0, partial=AssistantMessage(content=[]))
        assert event.type == "thinking_start"

    def test_thinking_delta_event(self):
        event = ThinkingDeltaEvent(
            content_index=0, delta="thinking...", partial=AssistantMessage(content=[]),
        )
    ...
    def test_thinking_end_event(self):
        event = ThinkingEndEvent(
            content_index=0, content="thought", partial=AssistantMessage(content=[]),
        )
    ...
    def test_tool_call_delta_event(self):
        event = ToolCallDeltaEvent(
            content_index=0, delta='{"q": "test"}', partial=AssistantMessage(content=[]),
        )
        assert event.type == "toolcall_delta"

    def test_tool_call_end_event(self):
        event = ToolCallEndEvent(
            content_index=0, tool_call=ToolCall(id="tc1", tool_name="search", arguments={}),
            partial=AssistantMessage(content=[]),
        )
