...
from nova_ai import AbortController, AbortSignal, builtin_models, get_builtin_model
...
from nova_ai.types import AssistantMessage
from nova_ai.types import AssistantMessage as AssistantMessageType
...
def _models():
    """构造包含内置 provider 的 Models 集合。"""
    return builtin_models()


def stream(model, context, options=None):
    """通过 Models 集合发起流式调用（兼容旧测试写法）。"""
    return _models().stream(model, context, options)


def stream_simple(model, context, options=None):
    """通过 Models 集合发起简化流式调用（兼容旧测试写法）。"""
    return _models().stream_simple(model, context, options)


async def complete(model, context, options=None):
    """通过 Models 集合完成非流式调用（兼容旧测试写法）。"""
    return await _models().complete(model, context, options)


async def complete_simple(model, context, options=None):
    """通过 Models 集合完成简化非流式调用（兼容旧测试写法）。"""
    return await _models().complete_simple(model, context, options)
...
class TestRealModelRegistry:
    ...
    def test_registry_lookup(self):
        if not os.environ.get("VOLCENGINE_API_KEY"):
            pytest.skip("VOLCENGINE_API_KEY not set")
        model = get_builtin_model("volcengine", "deepseek-v4-flash-260425")
    ...
    def test_all_volcengine_models_registered(self):
        ...
        for model_id in VOLCENGINE_MODELS:
            m = get_builtin_model("volcengine", model_id)
