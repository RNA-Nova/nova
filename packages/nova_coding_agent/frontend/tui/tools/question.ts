...
function renderMultiQuestion(
  ...
  // 线上词表：done/failed/cancelled 均终态（旧 error 含执行失败与中止两义）
...
export default function renderQuestion(input: RendererInput): Component {
  ...
  // item.args 线上为 unknown——按 question 参数形状窄化
  const status = input.item.status;
  const args = (input.item.args ?? {}) as Record<string, unknown> & {
    question?: string;
  ...
  if (labels.length > 0 && (status === 'pending' || status === 'running')) {
  ...
  // 线上词表：pending = 参数流式中（旧 streaming）；done/failed/cancelled = 终态（旧 error 拆为失败/中止）
  const terminal = status === 'done' || status === 'failed' || status === 'cancelled';
  if (status === 'pending' || terminal) {
    if (d.answer === undefined && !terminal) {
