...
export default function renderSubagent(input: RendererInput): Component {
  ...
  // 线上 item 状态（pending/running/done/failed/…）；parallel 段有同名局部文本变量，此处命名区分
  const itemStatus = input.item.status;
  ...
  /** 调用头部（pending 态或结果缺失时的标题——pi renderCall 对位）。 */
  function renderHeader(): Text {
    // item.args 线上为 unknown——按 subagent 三模式参数形状窄化
    const args = (input.item.args ?? {}) as {
      agent?: string;
      task?: string;
  ...
  return new (class extends Container {
    override render(width: number): string[] {
      ...
      // 无结果（pending/参数阶段或异常回执）：头部 + 内容文本兜底。
      if (results.length === 0) {
      ...
      const isLive = running > 0 && itemStatus !== 'done';
