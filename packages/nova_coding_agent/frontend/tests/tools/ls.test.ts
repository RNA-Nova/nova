...
function lsInput(overrides: Record<string, unknown>) {
  return { toolName: 'ls', status: 'done' as const, env, ...overrides } as any;
}
...
/** running 态 partial 回退的辅助（避免重复构造输入）。 */
function renderLsRender(status: 'running'): string {
  const lines = renderLines(
    renderLs(lsInput({ status, partial: { content: textContent('partial-entry/') } })),
  );
  return lines.join('\n');
}
