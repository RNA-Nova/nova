"""路径解析辅助函数（对齐 pi path-utils.ts 的 resolveReadPath 与 paths.ts）。"""
...
# Unicode 空格变体（对齐 pi paths.ts UNICODE_SPACES：归一为普通空格）
...
# AM/PM 前的普通空格（对齐 pi tryMacOSScreenshotPath 的 / (AM|PM)\./gi）
_AM_PM_SPACE = re.compile(r" (AM|PM)\.", re.IGNORECASE)


def normalize_input(path: str) -> str:
    """输入归一（对齐 pi normalizePath 的 normalizeUnicodeSpaces + stripAtPrefix）。"""
...
def _macos_path_variants(resolved: str) -> List[str]:
    """macOS 文件名变体重试链（对齐 pi resolveReadPath 的四级变体，按序）。

    顺序：AM/PM 窄不间断空格 → NFD 分解 → 弯引号 → NFD+弯引号组合；
    与 resolved 相同的变体跳过（对齐 pi 的 ``variant !== resolved`` 判定）。
    """
    variants: List[str] = []
...
def resolve_path(path: str, cwd: Optional[str] = None) -> str:
    ...
    输入先做 Unicode 空格归一与 ``@`` 前缀剥离（对齐 pi resolveToCwd）；
    解析结果不存在时按 macOS 文件名变体重试（对齐 pi resolveReadPath）。
    """
    if not path:
        return ""
