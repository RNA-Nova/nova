...
fn find_nova_executor_home_from_env(nova_executor_home_env: Option<&str>) -> std::io::Result<AbsolutePathBuf> {
        ...
        Some(val) => {
            ...
            if !metadata.is_dir() {
                Err(std::io::Error::new(
                    ...
                    format!("NOVA_EXECUTOR_HOME points to {val:?}, but that path is not a directory"),
                ))
            } else {
...
mod tests {
    ...
    fn find_nova_executor_home_env_missing_path_is_fatal() {
        ...
        let err = find_nova_executor_home_from_env(Some(missing_str)).expect_err("missing NOVA_EXECUTOR_HOME");
    ...
    fn find_nova_executor_home_env_file_path_is_fatal() {
        ...
        let err = find_nova_executor_home_from_env(Some(file_str)).expect_err("file NOVA_EXECUTOR_HOME");
    ...
    fn find_nova_executor_home_env_valid_directory_canonicalizes() {
        ...
        let resolved = find_nova_executor_home_from_env(Some(temp_str)).expect("valid NOVA_EXECUTOR_HOME");
    ...
    fn find_nova_executor_home_without_env_uses_default_home_dir() {
        let resolved =
            find_nova_executor_home_from_env(/*nova_executor_home_env*/ None).expect("default NOVA_EXECUTOR_HOME");
