...
use nova_executor_utils_path_uri::LegacyAppPathString;
use nova_executor_utils_path_uri::PathConvention;
...
const PROTECTED_METADATA_NOVA_EXECUTOR_PATH_NAME: &str = ".nova";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_NOVA_EXECUTOR_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer executor degrades to warn-and-ignore
...
/// 可写根路径的解析策略：Effective 完全规范化（历史行为）；
/// PreserveMutableComponents 只规范化受信任的顶层别名，保留攻击者可变的
/// 深层组件供平台沙箱（如 Seatbelt）自行锚定校验。
#[derive(Clone, Copy)]
...
impl FileSystemSandboxPolicy {
    ...
    fn get_writable_roots_with_cwd_impl(
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.nova -> <root>/decoy`, bwrap must still see
            // `<root>/.nova`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.nova` so downstream sandboxes can still detect and mask
...
/// 只规范化受信任的顶层系统别名（如 `/tmp -> /private/tmp`），不深追更深层
/// 的组件——深层组件可能被已在沙箱内运行的进程改写，这里跟随它们会把一次
/// 路径检查变成新的授权。
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .nova read-only to the agent by
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "NOVA_EXECUTOR_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry::new(
                path("file:///C:/workspace/.nova").into(),
                FileSystemAccessMode::Read,
            ),
        ...
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.nova/config"), &context),
            Some(".nova"),
        );
    }
    ...
    fn writable_roots_proactively_protect_missing_dot_nova() {
        ...
        let expected_dot_nova = expected_root.join(".nova");
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".nova/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let nova_dir = real_root.join(".nova");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&nova_dir).expect("create .nova");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let nova_dir = real_tmpdir.join(".nova");
    ...
    #[cfg(unix)]
    #[test]
    fn legacy_native_string_policy_json_round_trips_through_raw_seam() {
        // 线上兼容：迁移前的 rollout/配置 JSON 中 path 变体携带本机原生字符串，
        // 旧客户端与旧线上数据必须仍能反序列化，且经 Raw 接缝写回时形状不变。
        let legacy = serde_json::json!({
            ...
            "entries": [
                ...
                {
                    "path": {"type": "glob_pattern", "pattern": "/tmp/work/**/*.env"},
                    "access": "deny",
        ...
        let policy = legacy
            .to_string()
            .parse::<FileSystemSandboxPolicy>()
            .expect("legacy native-string policy should deserialize");
        assert_eq!(policy.entries.len(), 3);
        assert_eq!(
            policy.entries[0].path,
            FileSystemPath::Path {
                path: PathUri::from_host_native_path("/tmp/work").expect("path URI"),
            }
        );

        let raw = RawFileSystemSandboxPolicy::try_from(policy).expect("raw policy");
        assert_eq!(serde_json::to_value(raw).expect("json"), legacy);
    }

    #[cfg(unix)]
    #[test]
    fn raw_seam_rejects_paths_that_cannot_round_trip_losslessly() {
        // 有损检查的另一半：运行时策略里的 opaque URI（无原生路径表达）
        // 不允许写回旧字符串接缝，避免下游误读为一个无关的本机路径。
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry::new(
            PathUri::parse("file:///%00/bad/path/YQ")
                .expect("opaque URI")
                .into(),
            FileSystemAccessMode::Deny,
        )]);

        assert!(RawFileSystemSandboxPolicy::try_from(policy).is_err());
    }
}
