...
// ---------------------------------------------------------------------------
// HttpEgressPolicy（http/request 出网门控）
// ---------------------------------------------------------------------------

use super::HttpEgressPolicy;

fn policy(domains: &[&str]) -> HttpEgressPolicy {
    HttpEgressPolicy {
...
#[test]
fn http_egress_policy_empty_allowlist_allows_everything() {
    let policy = policy(&[]);
    assert!(policy.allows("https://example.com/api"));
    assert!(policy.allows("http://127.0.0.1:8080/health"));
}

#[test]
fn http_egress_policy_matches_exact_domain_and_subdomains() {
    let policy = policy(&["example.com", "internal.local"]);
    assert!(policy.allows("https://example.com/api"));
    assert!(policy.allows("https://api.example.com/v1"));
    assert!(policy.allows("http://deep.api.example.com/"));
    assert!(!policy.allows("https://notexample.com/"));
    assert!(!policy.allows("https://example.com.evil.com/"));
    assert!(policy.allows("https://internal.local/"));
}

#[test]
fn http_egress_policy_rejects_unparseable_urls() {
