...
impl ExecServerHandler {
    ...
    pub(crate) async fn environment_config_read(
        ...
        crate::environment_config::read_environment_config(params)
            .await
    ...
    pub(crate) async fn fs_write_stream(
        &self,
        params: FsWriteStreamParams,
    ) -> Result<FsWriteStreamResponse, JSONRPCErrorError> {
        self.require_initialized_for("filesystem")?;
        self.file_system.write_stream(params).await
    }

    pub(crate) async fn fs_write_stream_chunk(
        &self,
        params: FsWriteStreamChunkNotification,
    ) -> Result<(), String> {
        if let Err(err) = self.require_initialized_for("filesystem") {
            // 通知无回执：初始化前的 chunk 只告警忽略，不让协议错误关闭连接
            tracing::warn!(
                "ignoring fs/writeStream/chunk before initialize: {}",
                err.message
            );
            return Ok(());
        }
        self.file_system.write_stream_chunk(params).await
    }

    pub(crate) async fn fs_write_stream_done(
        &self,
        params: FsWriteStreamDoneParams,
    ) -> Result<FsWriteStreamDoneResponse, JSONRPCErrorError> {
        self.require_initialized_for("filesystem")?;
        self.file_system.write_stream_done(params).await
    }
