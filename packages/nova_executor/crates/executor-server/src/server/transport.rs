...
async fn websocket_upgrade_handler(
    ...
    if let Some(expected_token) = &state.auth_token {
        ...
        let valid = auth_header
            .map(|value| value.strip_prefix("Bearer ").is_some_and(|token| token == expected_token))
            .unwrap_or(false);
        if !valid {
            warn!(%peer_addr, "rejecting websocket connection with invalid or missing bearer token");
