...
async fn remote_file_system_sends_path_and_sandbox_cwd_uris_without_native_conversion() {
    ...
    for path in &paths {
        assert_eq!(
            file_system
                .read_file(path, ReadFileOptions::default(), Some(&sandbox))
                .await
...
async fn concurrent_remote_metadata_requests_share_only_in_flight_results() {
    ...
    let (first, second) = tokio::join!(
        file_system.get_metadata(&path, GetMetadataOptions::default(), /*sandbox*/ None),
        file_system.get_metadata(&path, GetMetadataOptions::default(), /*sandbox*/ None),
    );
...
async fn remote_metadata_requests_do_not_cross_path_or_sandbox_boundaries() {
    ...
    let (first, second) = tokio::join!(
        file_system.get_metadata(&first_path, GetMetadataOptions::default(), Some(&sandbox)),
        file_system.get_metadata(&first_path, GetMetadataOptions::default(), Some(&sandbox)),
    );
...
#[tokio::test]
async fn remote_read_file_passes_no_follow_on_the_wire() {
    let (websocket_url, captured_params, server) = record_read_file_params(2).await;
    let file_system = RemoteFileSystem::new(LazyRemoteExecServerClient::new(
        ExecServerTransportParams::websocket_url(
            websocket_url,
            DEFAULT_REMOTE_EXEC_SERVER_CONNECT_TIMEOUT,
    ...
    // follow 缺省（true）不下发字段；no-follow（false）显式下发 false
    assert_eq!(
        captured_params.await.expect("captured params"),
        vec![
            FsReadFileParams {
...
#[tokio::test]
async fn concurrent_remote_no_follow_metadata_requests_are_not_shared() {
    // no-follow 语义与 in-flight 共享缓存键（仅路径）不同源：两个并发 no-follow
    // 请求各自发 RPC（对比 follow 默认路径的同参数共享，见相邻测试）
    let (websocket_url, captured_params, server) = record_metadata_params(vec![
        MetadataResponse::Immediate(Ok(metadata_response(/*size*/ 42))),
        MetadataResponse::Immediate(Ok(metadata_response(/*size*/ 42))),
    ])
    ...
    let (first, second) = tokio::join!(
        file_system.get_metadata(&path, no_follow, /*sandbox*/ None),
        file_system.get_metadata(&path, no_follow, /*sandbox*/ None),
    );
    first.expect("first no-follow metadata request");
    second.expect("second no-follow metadata request");
