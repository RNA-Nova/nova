...
use nova_executor_protocol_core::capabilities::CapabilityRootLocation;
use nova_executor_protocol_core::capabilities::SelectedCapabilityRoot;
...
use crate::environment_toml::environment_provider_from_nova_executor_home;
...
impl EnvironmentManager {
    ...
    ///
    /// If `NOVA_EXECUTOR_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `CODEX_EXEC_SERVER_URL` behavior.
    pub async fn prepare_from_nova_executor_home(
        nova_executor_home: impl AsRef<std::path::Path>,
        ...
        let source = if let Some(config) = noise_environment_config_from_env()? {
            ...
            let provider = environment_provider_from_nova_executor_home(nova_executor_home.as_ref())?;
            PreparedEnvironmentSource::Snapshot(provider.snapshot().await?)
        };
        Ok(PreparedEnvironmentManager { source })
    }

    /// Builds a manager from `NOVA_EXECUTOR_HOME` with an explicit outbound HTTP policy.
    pub async fn from_nova_executor_home(
        nova_executor_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        http_client_factory: HttpClientFactory,
    ) -> Result<Self, ExecServerError> {
        Self::prepare_from_nova_executor_home(nova_executor_home)
            .await?
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = nova_executor_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = nova_executor_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            nova_executor_protocol_core::models::PermissionProfile::from_runtime_permissions(
                &nova_executor_protocol_core::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                nova_executor_protocol_core::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
