        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    codex_linux_sandbox_exe: self.runtime_paths.executor_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.executor_self_exe.as_path())
        .chain(runtime_paths.executor_linux_sandbox_exe.as_deref())
    {
...
fn spawn_command(
    ...
    env.retain(|name, _| !nova_executor_protocol_core::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        ...
        let runtime_paths =
            ExecServerRuntimePaths::new(executor_self_exe, /*executor_linux_sandbox_exe*/ None)
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = nova_executor_file_system::FileSystemSandboxContext::from_permission_profile(
