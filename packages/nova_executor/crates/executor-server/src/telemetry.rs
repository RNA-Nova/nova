...
// shell snapshot 捕获（executor 内缓存登录 shell 状态）——毫秒直方图 + 计数，
...
impl ExecServerTelemetry {
    ...
    pub(crate) fn shell_snapshot_captured(
        ...
        // 本地执行没有 exec-server 遥测归属方——优先显式配置的服务端 client，
        // 缺席时回退宿主全局 metrics client
        let Some(metrics) = self
            .inner
            .as_ref()
            .map(|inner| inner.metrics.clone())
            .or_else(nova_executor_otel::global)
        else {
            return;
        };
        let success = if result.is_ok() { "true" } else { "false" };
        let mut tags = vec![("version", "v2"), ("success", success)];
        let _ = metrics.record_duration(SHELL_SNAPSHOT_DURATION_METRIC, duration, &tags);
...
fn register_active_gauges(metrics: &MetricsClient, active: &Arc<Mutex<ActiveCounts>>) {
    for transport in [ConnectionTransport::Stdio, ConnectionTransport::WebSocket] {
