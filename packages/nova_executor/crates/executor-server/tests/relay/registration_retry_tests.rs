...
// nova 的 remote 注册路径没有移植 codex 的 register_environment_with_retry
//（remote.rs 里无 registration_retry 模块）：首次注册 503 冲突后直接报错退出，
// 不会带同一 Noise identity 重试。待重试逻辑落地后取消 ignore。
#[ignore = "nova 尚未实现注册冲突重试（register_environment_with_retry）"]
async fn registration_retries_preserve_noise_identity_and_initialized_session() -> Result<()> {
    ...
    assert_eq!(resumed.environment_info().await?, environment_info);
