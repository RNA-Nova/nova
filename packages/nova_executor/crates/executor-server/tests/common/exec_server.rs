//! exec-server 集成测试夹具：以子进程方式起真实服务器（WS 回环），
//! 并提供 JSON-RPC 收发、事件等待、可断连代理等测试原语。
//!
//! 对位 codex `exec-server/tests/common/exec_server.rs`；服务器本体就是
//! 当前测试二进制（`exec-server` 子命令经 `common/mod.rs` 的 ctor 分派）。
#![allow(dead_code)]
...
use nova_executor_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestExecutorHelperPaths,
    child: Child,
...
pub(crate) struct TestExecutorHelperPaths {
    pub(crate) executor_exe: PathBuf,
    pub(crate) executor_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_executor_helper_paths() -> anyhow::Result<TestExecutorHelperPaths> {
    let (helper_binary, executor_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestExecutorHelperPaths {
        executor_exe: helper_binary,
...
{
    let helper_paths = test_executor_helper_paths()?;
    let nova_executor_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.executor_exe);
    ...
    child.env("NOVA_EXECUTOR_HOME", nova_executor_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn nova_executor_home(&self) -> &std::path::Path {
        self.nova_executor_home.path()
    }
...
/// 服务器监听就绪后会在 stdout 打印一行 `ws://...`（见 server/transport.rs），
/// 夹具按行读取直到拿到 listen URL。
