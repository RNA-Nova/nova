...
use nova_executor_server::HttpHeader;
use nova_executor_server::HttpRedirectPolicy;
use nova_executor_server::HttpRequestBodyDeltaNotification;
use nova_executor_server::HttpRequestParams;
use nova_executor_server::HttpRequestResponse;
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<nova_executor_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
...
async fn exec_server_http_request_rejects_protected_environment_headers() -> anyhow::Result<()> {
    let mut server = exec_server_with_env(
        [(
            "NOVA_EXECUTOR_PROXY_ATTRIBUTION_TOKEN",
