...
use nova_executor_server::ExecOutputStream;
use nova_executor_server::ExecParams;
use nova_executor_server::ExecProcess;
...
#[cfg(unix)]
use nova_executor_server::ShellSnapshotRequest;
use nova_executor_server::StartedExecProcess;
use nova_executor_server::WriteStatus;
...
async fn shell_snapshot_v2_filters_profile_exports_and_stays_in_memory(
    ...
    if use_sandbox
        && let Some(warning) =
            nova_executor_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
    ...
    if let Some(server) = context._server {
        assert!(!server.nova_executor_home().join("shell_snapshots").exists());
    }
    Ok(())
}
...
async fn shell_snapshot_v2_capture_failure_falls_back_and_retries(
    ...
    if use_remote
        && let Some(warning) =
            nova_executor_sandboxing::system_bwrap_warning(&PermissionProfile::workspace_write())
    {
        eprintln!("skipping sandbox test: {warning}");
        return Ok(());
    }
