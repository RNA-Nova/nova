...
// Windows 稳定 ingress 集成测试（随 codex 一并带入）。
// 注意：Windows 共享代理 ingress（windows_proxy_ingress.rs）第一期未移植，
// `NetworkProxy::network_proxy_restricting_sid` 当前恒为 None——主测试因此标记
// #[ignore]，待 ingress 恢复批次一并恢复（见 src/lib.rs 裁点说明）。
// 相对 codex 的机械适配：crate 名 codex_windows_sandbox →
...
use nova_executor_network_proxy::ConfigReloader;
use nova_executor_network_proxy::ConfigReloaderFuture;
use nova_executor_network_proxy::ConfigState;
use nova_executor_network_proxy::NetworkDecision;
use nova_executor_network_proxy::NetworkMode;
use nova_executor_network_proxy::NetworkPolicyDecider;
use nova_executor_network_proxy::NetworkPolicyRequest;
use nova_executor_network_proxy::NetworkProtocol;
use nova_executor_network_proxy::NetworkProxy;
use nova_executor_network_proxy::NetworkProxyConfig;
use nova_executor_network_proxy::NetworkProxyState;
use nova_executor_network_proxy::build_config_state;
...
const CHILD_MODE_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_CHILD";
const HTTP_ADDR_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_HTTP_ADDR";
const SOCKS_ADDR_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_SOCKS_ADDR";
const ORIGIN_PORT_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_ORIGIN_PORT";
const ALLOWED_HOST_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_ALLOWED_HOST";
const DENIED_HOST_ENV: &str = "NOVA_EXECUTOR_WINDOWS_PROXY_TEST_DENIED_HOST";
...
#[ignore = "depends on the Windows shared proxy ingress (not yet ported)"]
