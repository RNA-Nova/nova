//! 上行连接客户端（移植自 codex network-proxy `upstream.rs`）。
//!
//! 裁点：`direct_with_tls_root_store` / `from_env_proxy_with_tls_root_store` 两个
//! 自定义根证书构造器服务于 MITM（伪造站点证书的可信校验），第一期未移植 MITM，
//! 一并裁掉；对应测试 `mitm_upstream_client_trusts_startup_custom_ca` 亦裁。
