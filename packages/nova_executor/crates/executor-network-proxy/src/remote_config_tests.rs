//! 移植自 codex network-proxy `remote_config_tests.rs`，按 nova 线上形状适配：
//! - `NetworkMode::Limited` → `NetworkMode::None`（nova 只有 None|Proxy）；
//! - `mitm_hooks` 为 `Vec<String>`；
//! - `NetworkProxyAuditMetadata` 只有 `conversation_id` / `turn_id`。
