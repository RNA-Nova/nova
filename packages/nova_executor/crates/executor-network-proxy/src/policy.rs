...
fn compile_globset_with_policy(
    ...
    for pattern in patterns {
        ...
        // 支持的域名模式：
        // - "example.com": 精确匹配
        // - "*.example.com": 匹配任意子域名（不含 apex）
        // - "**.example.com": 匹配 apex 与任意子域名
        // - "*": 仅在显式启用 allowlist 编译时匹配所有主机
...
impl DomainPattern {
    /// 为约束比较解析策略模式。
    ///
    /// glob 语法校验发生在构建 globset 时；这里只解码通配前缀，保持约束检查轻量。
