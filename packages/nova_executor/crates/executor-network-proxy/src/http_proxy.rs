//! HTTP 代理（移植自 codex network-proxy `http_proxy.rs`）。
//!
//! 裁点（第一期）：
//! - MITM 面：`ConnectMitmMode` / `mitm_connect_tunnel` / TLS 前缀嗅探未移植。
//!   CONNECT 隧道只做直接转发；`NetworkMode::None`（无网络访问）下 CONNECT 一律拒绝
//!   （codex 的 Limited 模式在裁掉 MITM 后语义退化为全拒）。
//! - 凭证经纪：明文凭证注入（`inject_plaintext_credentials_if_enabled`）未移植。
//! - `request_disconnect`：`NetworkPolicyRequest.disconnect` 字段与
//!   `track_http_request` 包装未移植（nova 线上 `NetworkPolicyRequest` 无该字段）。
...
pub async fn run_http_proxy(
    ...
    let listener = TcpListener::build()
        .bind(addr)
        .await
        // Rama 的 `BoxError` 是没有显式 `'static` 约束的 `Box<dyn Error + Send + Sync>`，
        // 不满足 `anyhow::Context` 的 `StdError` 约束。先包进 Rama 的 `OpaqueError`
        // 以保留原始错误链，同时能用 anyhow 串联上下文。
...
pub(crate) fn http_proxy_service(
    ...
    // 这个代理监听器只需要 HTTP/1 代理语义。用 Rama 的 auto builder 会让每个接入的
    // socket 先走 HTTP 版本嗅探预读路径，才可能进入代理请求解析；在 macOS 上这可能
    // 让某些本地客户端在 CONNECT/absolute-form 处理前就卡住。
...
async fn http_connect_accept(
    ...
    // 裁点（MITM）：codex 在 Limited 模式下要求 MITM 以便对隧道内请求执行方法策略；
    // nova 线上 `NetworkMode::None` 表示无网络访问，CONNECT 直接拒绝。
    if mode == NetworkMode::None {
        ...
        let client = client.as_deref().unwrap_or_default();
        warn!(
            "CONNECT blocked; network mode denies tunneling (client={client}, host={host}, mode={mode:?})"
        );
        return Err(blocked_text_with_details(REASON_METHOD_NOT_ALLOWED, &details));
    }
...
async fn http_connect_proxy(upgraded: Upgraded) -> Result<(), Infallible> {
    // 裁点（MITM）：裁掉 MITM 后 CONNECT 只有直接转发一种形态。
    let result: Result<(), OpaqueError> = forward_connect_tunnel(upgraded).await;
...
async fn http_plain_proxy(
    ...
    // `x-unix-socket` 是与本地守护进程通信的逃生门。保持严格限定：
    // 仅 macOS + 默认显式 allowlist，避免代理变成通用的本地权限提升机制。
    if let Some(unix_socket_header) = req.headers().get("x-unix-socket") {
        ...
        if !method_allowed {
            ...
            warn!(
                "unix socket blocked by method policy (client={client}, method={method}, mode=none)"
            );
    ...
    // 裁点（MITM）：codex 在此处对 hook 命中的主机强制 MITM；未移植。
    ...
    if !method_allowed {
        ...
        let method = req.method();
        warn!(
            "request blocked by method policy (client={client}, host={host}, method={method}, mode=none)"
        );
        return Ok(json_blocked(
            &host,
            REASON_METHOD_NOT_ALLOWED,
    ...
    // 先剥 hop-by-hop 头，再把请求交给上游（策略相关元数据已提取完毕）。
...
fn remove_hop_by_hop_request_headers(headers: &mut HeaderMap) {
    ...
    // 0x74,0x65 是 ASCII "te"（HTTP TE hop-by-hop 头）。
...
mod tests {
    ...
    #[tokio::test]
    async fn http_connect_accept_blocks_in_none_mode() {
        // 适配：codex 用 Limited 模式验证 CONNECT 需要 MITM；nova 的 None（无网络访问）
        // 在裁掉 MITM 后一律拒绝 CONNECT（method policy）。
        let policy = {
        ...
        state.set_network_mode(NetworkMode::None).await.unwrap();
    ...
    #[tokio::test]
    async fn http_connect_accept_allows_allowlisted_host_in_proxy_mode() {
        let policy = {
    ...
    // 裁点：codex 的 brokered MITM 延迟探测、明文凭证注入、hook 命中阻断等测试
    // 依赖 MITM / 凭证经纪面，第一期未移植，随之裁掉。

    #[tokio::test]
    async fn connect_forwards_server_first_opaque_protocol() {
        // 改写自 codex `brokered_connect_forwards_server_first_opaque_protocol_without_mitm`：
        // 去掉凭证经纪配置，验证 Proxy 模式下非 TLS 上行协议的直接隧道转发。
