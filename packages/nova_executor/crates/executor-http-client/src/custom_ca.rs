...
//!
//! - read CA material from `NOVA_EXECUTOR_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
pub const NOVA_EXECUTOR_CA_CERT_ENV: &str = "NOVA_EXECUTOR_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set NOVA_EXECUTOR_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// CA handling before finally constructing the client. `NOVA_EXECUTOR_CA_CERTIFICATE` takes precedence
...
/// `NOVA_EXECUTOR_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
trait EnvSource {
    ...
    ///
    /// `NOVA_EXECUTOR_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Nova-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(NOVA_EXECUTOR_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: NOVA_EXECUTOR_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
mod tests {
    ...
    use super::NOVA_EXECUTOR_CA_CERT_ENV;
    use super::EnvSource;
    ...
    fn ca_path_prefers_nova_env() {
        let env = map_env(&[
            (NOVA_EXECUTOR_CA_CERT_ENV, "/tmp/nova.pem"),
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(NOVA_EXECUTOR_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
