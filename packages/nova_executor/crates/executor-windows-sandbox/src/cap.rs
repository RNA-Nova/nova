...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.nova`)
...
mod tests {
    ...
    fn write_roots_get_path_scoped_sids() {
        ...
        assert_eq!(
            extra_sid,
            writable_root_cap_sid_for_path(&sandbox_home, &extra_root).expect("extra root sid again")
        );
