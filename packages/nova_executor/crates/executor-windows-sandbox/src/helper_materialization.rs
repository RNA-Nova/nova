...
pub(crate) const RESOURCES_DIRNAME: &str = "nova-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "nova-command-runner.exe",
        }
    }
...
mod tests {
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "nova-command-runner.exe")
                .expect("helper path");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "nova-command-runner-test-suffix.exe");
    }
}
