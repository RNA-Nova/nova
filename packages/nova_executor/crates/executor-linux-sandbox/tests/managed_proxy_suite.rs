//! managed proxy 集成套件（移植自 codex linux-sandbox/tests/suite/managed_proxy.rs）。
//! 耦合面本就最小——codex 侧驱动直调 codex-linux-sandbox 二进制 CLI；nova 侧
//! 同款直调 nova-linux-sandbox（CARGO_BIN_EXE 同包注入）。测试体逐行保留。

#![cfg(target_os = "linux")]
#![allow(clippy::unwrap_used)]

use nova_executor_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
use nova_executor_network_proxy::write_attribution_frame;
...
// bwrap 缺失/权限不足时的可跳过信号（spawn 失败或 stderr 含这些字样）
...
/// bwrap 不可用判定：stderr 含 bwrap 缺失/权限字样
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    is_bwrap_unavailable_output(&output)
        || (!output.status.success()
            && is_managed_proxy_permission_error(&String::from_utf8_lossy(&output.stderr)))
}
...
fn linux_sandbox_command(
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_nova-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn unsupported_system_bwrap_falls_back_to_bundled_bwrap() {
    ...
    let Some(system_bwrap) = nova_executor_sandboxing::find_system_bwrap_in_path() else {
    ...
    let sandbox_executable = tempdir.path().join("nova-linux-sandbox");
    let original_executable = env!("CARGO_BIN_EXE_nova-linux-sandbox");
    ...
    let resources_dir = tempdir.path().join("nova-resources");
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_helper_dir = std::path::Path::new(env!("CARGO_BIN_EXE_nova-linux-sandbox"))
        .parent()
...
fn handoff_client() {
    let Ok(label) = std::env::var("NOVA_TEST_HANDOFF_LABEL") else {
        return;
    };
    ...
    let shared = std::env::var("NOVA_TEST_HANDOFF_SHARED").expect("shared directory");
    let shared = Path::new(&shared);
    let peer = std::env::var("NOVA_TEST_HANDOFF_PEER").expect("peer label");
...
async fn handoff_isolates_concurrent_endpoints_and_closes_privileged_descriptors() {
    ...
    // nova 套件文件即集成测试 crate 根（无模块前缀）——selector 直接用测试名
    // （codex 侧经 module_path 拆前缀，因其套件挂在 tests/all.rs 的 mod 下）
    let client_test = "handoff_client".to_string();
    ...
    for (label, peer, profile) in [
        ...
        env.insert("NOVA_TEST_HANDOFF_LABEL".to_string(), label.to_string());
        env.insert("NOVA_TEST_HANDOFF_PEER".to_string(), peer.to_string());
        env.insert(
            "NOVA_TEST_HANDOFF_SHARED".to_string(),
