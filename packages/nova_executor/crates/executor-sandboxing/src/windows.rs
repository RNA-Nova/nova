...
pub fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    let additional_deny_read_paths = nova_executor_windows_sandbox::resolve_windows_deny_read_paths(
        &file_system_sandbox_policy,
        sandbox_policy_cwd,
    )?;
    #[cfg(not(target_os = "windows"))]
    let additional_deny_read_paths: Vec<nova_executor_utils_absolute_path::AbsolutePathBuf> = Vec::new();
