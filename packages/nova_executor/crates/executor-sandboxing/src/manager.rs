...
use crate::landlock::NOVA_EXECUTOR_LINUX_SANDBOX_ARG0;
...
impl SandboxManager {
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let executor_home = nova_executor_utils_home_dir::find_nova_executor_home()
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_executor_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        executor_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    let helper =
        nova_executor_windows_sandbox::resolve_exe_for_launch(source.as_path(), executor_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        nova_executor_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(NOVA_EXECUTOR_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        NOVA_EXECUTOR_LINUX_SANDBOX_ARG0.to_string()
    }
}
