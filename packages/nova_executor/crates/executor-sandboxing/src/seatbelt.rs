...
use nova_executor_network_proxy::PROXY_URL_ENV_KEYS;
use nova_executor_network_proxy::has_proxy_url_env_vars;
use nova_executor_network_proxy::proxy_url_env_value;
...
use nova_executor_protocol_core::protocol::WritableRoot;
    ...
    for (index, access_root) in roots.into_iter().enumerate() {
            ...
            access_root.excluded_subpaths.into_iter().enumerate()
        {
            ...
            for (excluded_param, excluded_subpath) in excluded_subpaths {
                ...
                // protected directory itself, such as `mkdir .nova`.
