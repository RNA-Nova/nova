...
use nova_executor_utils_pty::SpawnedProcess;
use nova_executor_utils_pty::TerminalSize;
...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let executor_home = nova_executor_utils_home_dir::find_nova_executor_home()
                .context("windows sandbox: failed to resolve nova executor home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return nova_executor_windows_sandbox::spawn_windows_sandbox_session_for_level(
    ...
    if request.tty {
        nova_executor_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        nova_executor_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        nova_executor_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,
