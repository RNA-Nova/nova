...
import { PermissionDecisionCard } from './lib/permission-decision.js';
...
export default function extension(api: ExtensionUIAPI): void {
  ...
  // permission_decision 审批留痕卡（permission_gate 扩展的呈现——问记分离：
  // 对话框问、条目记；custom 条目通道，结构性不进 LLM 上下文）
  api.registerEntryRenderer('permission_decision', (entry) => new PermissionDecisionCard(entry.data));
}
